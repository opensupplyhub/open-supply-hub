...
### Code/API changes
* [OSDEV-3357](https://opensupplyhub.atlassian.net/browse/OSDEV-3357) - Claims dashboard v2 pilot polish (all behind the `enable_claims_v2_dashboard` switch): `/dashboard/claims-v2?claim={id}` now deep-links to a specific claim — the queue preselects it on load, and the browser URL mirrors the current selection as a shareable link (this is the hop from a Claims Tracker Jira ticket, and the format the eventual old-route redirect will map ids onto); refreshing the queue after an action keeps the current list on screen instead of blanking the page (full-page spinner only before the first load); the message templates' action links (e.g. the SLC address-update form) are built from the current environment's origin, so a message composed on Test no longer directs the claimant to production. Claim-details notes now carry `is_automated` (author is the claims-automation account, configurable via `CLAIMS_AUTOMATION_ACCOUNT_EMAIL`), and the queue's reply-window stage ignores automated messages — the pipeline's reminder emails no longer flip an overdue claim back to "awaiting claimant" on reminder day.
