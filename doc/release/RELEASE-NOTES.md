...
* [OSDEV-3525](https://opensupplyhub.atlassian.net/browse/OSDEV-3525) - Updated the local-development `minio` service image from `quay.io/minio/minio` to `public.ecr.aws/minio/minio` (same pinned tag `RELEASE.2025-09-07T16-13-09Z`). Quay.io began blocking anonymous pulls on 2026-09-23, breaking `start_code_quality_dev` and every other workflow that runs `docker compose up` locally. The AWS ECR Public Gallery mirror allows anonymous pulls from any environment without credentials. The long-term fix (mirroring to OS Hub's own ECR account) is tracked in [OSDEV-3453](https://opensupplyhub.atlassian.net/browse/OSDEV-3453).
