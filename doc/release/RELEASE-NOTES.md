...
* [OSDEV-3062](https://opensupplyhub.atlassian.net/browse/OSDEV-3062) - Fixed `GET /api/product-types/` raising `pylibmc.TooBig` on every request: the response payload (standard product types plus all distinct contributor-submitted values) exceeds memcached's per-item size limit, so the `cache_page` write failed and the endpoint was never cached, defeating the 10-minute filter-option caching added in 2.27.0 ([OSDEV-2948](https://opensupplyhub.atlassian.net/browse/OSDEV-2948)). The endpoint was converted from a function-based `api_view` to a `ProductTypes(APIView)` class using the reusable `cache_view_response` decorator (`api/view_response_cache.py`), which zlib-compresses the response before caching and serves it uncached (fail-open) if the compressed payload still exceeds `VIEW_RESPONSE_CACHE_MAX_BYTES`. The URL name `product_types` is unchanged. Mirrors the fix applied to `/api/parent-companies/` in [OSDEV-3030](https://opensupplyhub.atlassian.net/browse/OSDEV-3030).
