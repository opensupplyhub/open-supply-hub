...
### Bugfix
* [OSDEV-3506](https://opensupplyhub.atlassian.net/browse/OSDEV-3506) - The Dedupe Hub gazetteer now actually indexes facilities created since it was built, so a new upload can match production locations added after the service last started. `GazetteerCache` is documented as updating itself with any `Facility` rows added since the previous `get_latest()` call, but its incremental path indexed **at most one** facility and then re-indexed that same one indefinitely — three defects introduced porting the class from Django to SQLAlchemy: the version markers stored an unexecuted `Query` object rather than a scalar (no `.scalar()`), so the change guard compared two Query objects and was always true; the history query filtered `history_id == marker` where the Django original used `history_id__gt`, which SQLAlchemy 1.4 coerced into `history_id = (SELECT max(history_id) …)` and so returned a single row; and the rows were ordered `desc()` rather than ascending, leaving the marker on the oldest row of each batch. A full rebuild only happens on a cold start (`if cls._gazetter is None`), and Dedupe Hub is a long-lived Kafka consumer holding the gazetteer in memory for the life of the ECS task, so in practice every facility created since that task started was invisible to matching and overlapping uploads silently minted duplicate OS IDs. Also fixed alongside: `_get_new_match_history` read its version from `HistoricalFacility` instead of `HistoricalFacilityMatch` (inherited from the Django original), and `_match_version` advanced only for confirmed matches — with `>` restored that would strand the marker at the last `CONFIRMED` row and make every refresh re-read a growing tail of history, so it now advances for every row read. Does not retroactively merge duplicates already created; existing ones still need a moderator merge.
