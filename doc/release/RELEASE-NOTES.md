...
* `0227_add_isic_to_facility_index.py` - Adds `isic_section`, `isic_division`, `isic_group`, and `isic_class` array columns to `FacilityIndex`, plus SQL helpers to extract normalized ISIC Rev 4 codes from `isic_4` extended fields during indexing.
* `0228_create_isic_taxonomy_config.py` - Creates the singleton `IsicTaxonomyConfig` table with an inactive default row. No taxonomy is published until staff upload and publish via Django admin.
* `0229_index_facility_processing_raw_values.py` - Replaces `index_facility_type()` and `index_processing_type()` so unmatched raw contributor values from profile-visible ExtendedFields are indexed for search, while keeping standardized taxonomy slots from `matched_values`.
* `0230_create_facility_processing_value_index.py` - Creates `api_facility_processing_value`, a precomputed index of distinct facility and processing type values with facility counts, plus triggers that keep it synchronized with `FacilityIndex`. Empty values and common contributor placeholders such as `null`, `none`, and `unknown` are excluded.
...
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added singleton `IsicTaxonomyConfig` (pk=1) to track the active ISIC Rev 4 taxonomy version, S3 artifact keys, hierarchy counts, enable/disable state, publish audit fields, and the most recent publish error.

### Code/API changes
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/taxonomy-counts/?kind=facility_processing|isic4` to return per-node facility counts for the main search UI. `kind=facility_processing` responses are cached in Memcached for one hour; `kind=isic4` uses the default view-cache TTL. Extended `GET /api/facilities/` with a repeatable `isic_4` query parameter (`section:A`, `division:01`, `group:011`, `class:0111`) that filters against the new indexed ISIC columns. Extended `GET /api/facilities/` facility/processing type filters to accept taxonomy labels or free text: taxonomy values still use exact array overlap; unmatched terms of at least three characters use accent-insensitive whole-word or word-prefix matching across both indexed columns, excluding matches that occur only in the middle of a word. Facility list results rank whole-word matches before word-prefix matches, then apply the requested `sort_by` order. Empty facility/processing values are ignored, while unrecognized manual values shorter than three trimmed characters return no matches instead of fuzzy-resolving; declared taxonomy aliases remain supported. A repeated `processing_type_exact` companion selects the identical case- and punctuation-sensitive stored processing value and combines with other processing values using OR semantics. Unmatched raw contributor values from profile-visible ExtendedFields are now indexed in `FacilityIndex.facility_type` and `FacilityIndex.processing_type`; taxonomy counts continue to include only known taxonomy labels.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/processing-type-suggestions/`, which returns ranked processing type suggestions from `api_facility_processing_value`. Results include the exact stored value as the display label, taxonomy membership, parent facility types, facility count, and whether a result should be visually de-emphasized. Casing and punctuation variants remain separate suggestions with their individual counts. Selected facility types boost related suggestions but never exclude unrelated or contributor-supplied values.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Flat `GET /api/sectors/` responses, including contributor embed responses, now return only submitted strings that exactly match a curated `Sector.name`. Valid ungrouped sectors and submitted `Unspecified` remain available; grouped responses are unchanged.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/taxonomy-config/` to expose runtime ISIC Rev 4 settings (`enabled`, `version`, `taxonomyUrl`) for the search UI; responses are cached server-side for 60 seconds and invalidated on publish or enable/disable. Added `GET /api/taxonomy/isic4/` so Django serves the active taxonomy JSON from the private files bucket only after staff publish via admin. The search UI fetches `/api/taxonomy-config/` at runtime (Redux) to gate the ISIC filter and load the versioned taxonomy URL; taxonomy JSON responses use long-lived browser cache with a `?v=` query param for cache busting on publish.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added an ISIC taxonomy admin page at `/admin/api/isictaxonomyconfig/` (listed under **Api → ISIC taxonomies** in the Django sidebar) for staff with `api.change_isictaxonomyconfig` permission to upload CSV/XLSX/ODS taxonomy spreadsheets (11-column ISIC Rev 4 schema), preview the parsed Section → Division → Group → Class hierarchy, atomically publish validated JSON artifacts to versioned S3 prefixes, and enable or disable the search UI without deleting published versions. The React search UI fetches taxonomy JSON from Django and hides the ISIC filter and the "Match both facility type and ISIC categories" combine checkbox when the taxonomy is disabled; ISIC display on facility profiles is unchanged.
...
### What's new
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - On the main facility search page (non-embed), added an ISIC Rev 4 hierarchical search filter and upgraded Processing Type to a flat typeahead that searches both standard taxonomy terms and contributor-supplied values. Processing Type suggestions show parent facility types and facility counts, group non-taxonomy values separately, and visually de-emphasize—but do not hide—values unrelated to the selected Facility Type. Facility Type remains a separate dropdown. A new "Match both facility type and ISIC categories" checkbox lets contributors require both filters to match (AND semantics). Embedded maps keep the existing dropdown filters. Typing at least three characters and clicking Search commits unmatched input as a free-text facility/processing filter chip.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Staff can upload and publish updated ISIC Rev 4 taxonomies from Django admin without a code deploy; the search UI loads the active version from Django at runtime.

### Architecture/Environment changes
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Removed committed generated taxonomy artifacts (`isicRev4Taxonomy.js`, `isic_rev4.json`) and browser-facing files-bucket CORS/presigned-URL delivery. ISIC taxonomy JSON lives only in the private files bucket and is served through Django after staff publish it from admin.
...
### Bugfix
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Fixed facility and processing taxonomy searches returning locations whose matching value came only from an inactive source and was therefore absent from the location profile. Standardized taxonomy values and unmatched free text now follow the same profile-visibility rules; the targeted facility/processing backfill removes previously indexed hidden values.
...
* Ensure that the following commands are included in the `post_deployment` command:
    * `migrate`
    * `reindex_database`
* After migration `0226` deploys, run a one-time targeted backfill so existing unmatched raw facility/processing values become searchable and hidden values from inactive sources are removed:
    * `backfill_facility_index --fields facility_type,processing_type --parallel 10 --batch-size 10000`
* After the targeted facility/processing backfill completes, rebuild the processing type suggestion aggregate:
    * `recompute_facility_processing_value_index`
* Include the following command in `post_deployment` to invalidate one-year facility and facility-grid tile caches after deploying the updated filter semantics:
    * `incrementtileversion`
* The combined `backfill_facility_index` operation runs with no application downtime; monitor RDS CPU and connections on the first run after deploy and reduce `--parallel` if needed.
