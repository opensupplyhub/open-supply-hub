...
* [OSDEV-2786](https://opensupplyhub.atlassian.net/browse/OSDEV-2786) - Trade union-linked data is now withheld from OS Hub's paid products while staying visible on the public web app. A new `ShouldMaskContribution` service caches the set of `Union` contributors whose admin has `hide_in_paid_products` enabled, matching contributions by contributor `id` or `admin_id`. For programmatic (token-authenticated) requests, `GET /api/facilities/` (list + detail) and `GET /api/facilities-downloads/` relabel a masked contributor's name to `Other`, drop its contributor id, and strip its list name/description and other list metadata. This covers the `contributors` array, extended fields, names, addresses, sectors, claim info, alternate locations, and the `created_from` attribution (matched by name, since that record carries no contributor id). Web-client and embed traffic is unaffected; instead, the search-page download button is disabled (with an explanatory tooltip) when a search targets unions via the **Data Contributor Type** = `Union` filter, a union **Data Contributor**, or that contributor's **Contributor List** sub-filter.
