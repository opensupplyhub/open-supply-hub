...
#### Schema changes
* [OSDEV-3095](https://opensupplyhub.atlassian.net/browse/OSDEV-3095) - New `LeiMapping` model (`api_leimapping`): the internal audit ledger for GLEIF OS Hub→LEI mappings — `os_id` (unique, canonical), `lei`, `match_type` (`facility_name`/`parent_company`), `matched_name`, `score`, `mapping_file_date`, `status` (`active`/`removed`/`blacklisted`), timestamps. Never exposed publicly; answers "why does this facility have this LEI?" and drives idempotent ingestion.

### Code/API changes
* [OSDEV-3095](https://opensupplyhub.atlassian.net/browse/OSDEV-3095) - Added the `ingest_lei_mappings` management command (GLEIF/GODIN pilot). It reads an OS Hub→LEI mapping CSV (`--file`, `--contributor-id`, `--file-date`, `--dry-run`, `--batch-size`) and materializes each mapping as a real contribution chain attributed to the GLEIF contributor — one `FacilityList` + `Source` (`create=False`, so Dedupe Hub/geocoding are never triggered) per file, and per facility a `FacilityListItem` (`CONFIRMED_MATCH`) + active `FacilityMatch` (`CONFIRMED`) + `lei_id` `ExtendedField` (`{'raw_value': <LEI>, 'match_type': ...}`). Runs diff against the `LeiMapping` ledger: create / update-in-place / ledger-only metadata refresh (new monthly file date fires no ExtendedField reindex triggers) / removal (extended field deleted, contribution records kept as history, ledger row marked `removed`) / blacklisted rows never recreated. Merged OS IDs resolve via `FacilityAlias` to the canonical facility; unknown OS IDs, invalid LEIs, and duplicate targets are skipped and counted. Website display of LEIs remains gated by the existing `show_additional_identifiers` switch (OFF in production); note the switch does not gate API responses — that decision is tracked on OSDEV-3096 before any production import.
