...
### Bugfix
* [Hotfix] Dedupe Hub - Pinned the transitive dependency `BTrees` to 6.4. BTrees 6.5 (released 2026-08-20) removed the long-deprecated `byValue()` method that dedupe 1.9.4's canopy index calls while building the gazetteer, so every container image built on or after that date failed its gazetteer build at startup with the misleading error "Attempting to block with an index predicate without indexing records" — dedupe-hub ran but could not match (observed on Test after its Aug 26 redeploy and on Preprod during 2.29 validation). Long-lived containers built earlier, including Production's, were unaffected because they carry an older BTrees. BTrees was previously unpinned, so image rebuilds silently picked up the incompatible version; see [zopefoundation/BTrees#226](https://github.com/zopefoundation/BTrees/issues/226). Follow-up: pin all remaining transitive dependencies of dedupe 1.9.4, which is frozen upstream and will not adapt to future releases.
