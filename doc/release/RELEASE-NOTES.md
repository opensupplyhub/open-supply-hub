...
* `0228_add_polygons_switch.py` - Adds the `polygons` waffle switch (off by default), which gates the polygon section of the Django admin.
* `0229_add_india_labour_line_switch.py` - Adds the `india_labour_line_helpline` waffle switch (off by default), which gates the India Labour Line helpline partner field: the system provider, the contributor-profile spotlight, and the partner search filter.

#### Schema changes
* [OSDEV-3142](https://opensupplyhub.atlassian.net/browse/OSDEV-3142) - Added the `is_anonymized` column to `api_source`.

### Code/API changes
* Added the India Labour Line helpline as a system partner field (`india_labour_line_helpline`), mirroring the MIT Living Wage pattern but keyed on `Polygon` boundaries instead of US counties: production locations in India whose coordinates fall inside the `india_labour_line_helpline_areas` polygon, and that work in a covered sector (Apparel, Apparel Accessories, Footwear, Home Textiles, Leather, Textiles), get the helpline phone number (read at request time from the partner field's JSON schema `properties.phone_number.default`, so the number is editable in the Django admin without a deploy). The partner's contributor-profile spotlight and the Spotlight Data Partners search filter both resolve via PostGIS polygon containment. Everything is gated behind the `india_labour_line_helpline` waffle switch; missing configuration (polygon or schema) disables the feature with a logged warning rather than erroring or over-matching. Requires no front-end changes.
