...
* [OSDEV-3411](https://opensupplyhub.atlassian.net/browse/OSDEV-3411) - `SubmissionQualityProcessor` now logs the SLC submission body (the request payload as received, before cleaning) at `INFO` each time the LLM quality check runs, together with the contributor ID and the warning types raised (empty when nothing was flagged), and again when a resubmission bypasses the check via `ignore_warnings=true`. Comparing the two lines for a contributor in CloudWatch shows whether the warnings led to different data being submitted. No response or persistence changes.
