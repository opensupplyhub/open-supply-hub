...
* [OSDEV-2413](https://opensupplyhub.atlassian.net/browse/OSDEV-2413) - **[Release] Init** and **[Release] Deploy** now authenticate with `GITHUB_TOKEN` instead of a user-owned `PAT`. Because `GITHUB_TOKEN` pushes do not start other workflows, Init creates the `releases/v.*` branch and then `workflow_dispatch`es **Deploy to AWS** to Pre-prod; Deploy still creates the env tag and dispatches **Deploy to AWS** via the API. Same-repo `contents: write` and `actions: write` are set on those workflows.
