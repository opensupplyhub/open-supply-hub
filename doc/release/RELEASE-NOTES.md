...
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Enabled `pgaudit` database activity auditing on the RDS for PostgreSQL instances, resolving the Vanta SOC 2 test `aws-rds-pgaudit-enabled`. `aws_db_parameter_group.default` in `deployment/terraform/database.tf` now sets `shared_preload_libraries` to `pg_stat_statements,pgaudit` and `pgaudit.log` to `ddl,role`, both with `apply_method = "pending-reboot"`. The two values come from new `rds_shared_preload_libraries` and `rds_pgaudit_log` variables and are deliberately not overridden per environment, so every environment audits the same statement classes: schema changes and privilege/role grants. `read` and `write` are excluded because logging every SELECT, or every write on the ingestion path, would multiply log volume. `pg_stat_statements` — loaded by default on PostgreSQL 11 and later — is listed explicitly because setting `shared_preload_libraries` in a custom parameter group replaces the family default outright, and omitting it would silently disable query statistics. `pgaudit` was also added to the `install_db_exts` management command, which is idempotent, so `CREATE EXTENSION pgaudit` is a repeatable, reviewable step rather than a hand-run SQL statement. All six environments run a single-instance RDS PostgreSQL 16 database and there is no Aurora cluster anywhere in the configuration, so the ticket's Aurora acceptance criteria do not apply. **Each RDS instance must be rebooted manually after the Terraform apply**: `shared_preload_libraries` is a static parameter, and RDS does not reboot an instance on its own for a parameter change, not even during the maintenance window. Every environment runs `rds_multi_az = false`, so this is a hard restart with roughly 30-120 seconds of downtime rather than a failover. `install_db_exts` must run after that reboot — before it, the library is not loaded and the `pgaudit` line of the command fails with `pgaudit must be loaded via shared_preload_libraries`, which is logged while the other extensions still install. PostgreSQL logs remain unexported to CloudWatch Logs (`cloudwatch_logs_exports` is not passed to the `database_enc` module in any environment), so audit records land in RDS-local log files under RDS's own short retention; centralizing them is out of scope for this ticket. See `doc/ops/database-auditing.md` for the per-environment runbook and verification queries.
