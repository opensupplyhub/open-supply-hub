...
* [OSDEV-3216](https://opensupplyhub.atlassian.net/browse/OSDEV-3216) - ContriBot no longer fails on three contributor file shapes that were mishandled by the Airflow implementation and carried into the Lambda rebuild unchanged. A file of more than 10,000 rows raised `AttributeError: no attribute 'df_config'` because the row-limit check called `_add_diagnosis` before the error-code config and finding lists were initialised; those are now set up before the dataframe is read. A file with no `address` column raised `AttributeError: no attribute 'address'` in `check_name_address_duplicates` and `check_for_country_name_in_address`, aborting the whole run even though the missing column had already been recorded as `T0003`; both now check the column exists. The unrecognised-column finding (`T0008`) passed `column=` where the signature takes `column_name=`, leaving the finding's column name blank — both are now passed, since the record needs one and the message template renders the other. Regression tests cover all three.
