...
* [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Made `GET /health-check/` app liveness only (HTTP 200, body `ok`) — no django-watchman Postgres/cache checks. Removed `django-watchman`. BetterStack, ALB, and ECS continue to use `/health-check/`; DB/cache degradation is covered by CloudWatch (including new ALB `HTTPCode_Target_5XX_Count`, ALB `TargetResponseTime`, and RDS `DatabaseConnections` alarms on `topic…GlobalNotifications`). `OriginSourceMiddleware` and `DarkVisitorsMiddleware` skip `/health-check/`. See `doc/ops/monitoring.md`. **Post-deploy:** if BetterStack still expects the old watchman JSON body, update it to accept `ok`; confirm SNS subscriptions for the new alarms.
