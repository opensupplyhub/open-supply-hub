...
* [OSDEV-2996](https://opensupplyhub.atlassian.net/browse/OSDEV-2996) - Migrated the React app's build toolchain from `react-scripts` 4 / webpack 4 (Create React App, unmaintained) to **Vite**, keeping Jest as the test runner. This removes the last dependencies locked to the dead CRA 4 tree and clears the associated vulnerability findings that could not otherwise be patched (webpack-dev-server, webpack-dev-middleware, postcss 7.x, transitive `uuid`/`fast-xml-parser`, and the no-patch `ip` and `elliptic` — the latter two leave the dependency tree entirely under Vite rather than being suppressed). The `--openssl-legacy-provider` shim added in OSDEV-2995 is removed (no webpack 4 MD4 hashing on node 22).
  ...
  * **Build tooling:** bumped `sass` to ^1.101.0 (Vite 8 requires the modern Dart Sass compiler API) and modernized the two SCSS files to `@use 'sass:math'` / `math.div` / `math.percentage`.
