...
* `0235_add_facility_candidate_fields.py` - Adds five candidate columns (`is_candidate`, `polygon`, `confidence`, `external_id`, `source`) to `api_facility` and `api_historicalfacility`, builds their indexes `CONCURRENTLY`, and attaches two constraints: `api_facility_source_external_id_uniq` (unique on `(source, external_id)`) and `api_facility_source_requires_external_id` (CHECK: a row with a non-empty `source` must carry an `external_id`, so sourced rows always have an idempotency key). Non-atomic migration. Only the index builds avoid write blocking entirely (they scan both large tables and take a while); the `ALTER TABLE` statements take brief `ACCESS EXCLUSIVE` locks guarded by a 5s `lock_timeout` — if `migrate` fails with a lock timeout during deployment, rerun it (every statement is idempotent). See OSDEV-3242.
