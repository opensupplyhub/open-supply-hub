...
* [OSDEV-2665](https://opensupplyhub.atlassian.net/browse/OSDEV-2665) - Added DMARC `_dmarc` TXT records in Route 53 for all three domains to enable external compliance monitoring via `inbound.axl.net`: `opensupplyhub.org` (`p=reject`, replaces the previous `aws-accounting@opensupplyhub.org` `rua` address), `openapparel.org` (`p=none`, adds `rua` reporting address to the existing record), and `os-hub.net` (`p=none`, new record). Managed unconditionally in `deployment/terraform/dns.tf`.
