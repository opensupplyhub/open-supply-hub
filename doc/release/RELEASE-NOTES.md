...
* [OSDEV-2159](https://opensupplyhub.atlassian.net/browse/OSDEV-2159) - Fixed extended fields keeping the original uploader as contributor after a list's `Source.contributor` was reassigned in Django admin. Attribution such as "*<date>* by *<contributor>*" on the production location profile, in facility search responses and in downloads continued to name the previous contributor, because only `Source.contributor` was updated while the `ExtendedField.contributor` rows it produced were left behind. `Source.save()` now detects a contributor change and, in the same transaction, re-attributes every `ExtendedField` reached through `FacilityListItem.source` — covering `SINGLE` sources as well as `LIST` sources — via the new `SourceService.reassign_extended_fields`. The reassignment uses a queryset update on `contributor` only, so `updated_at` is unchanged (the publicly displayed contribution date is preserved) and no new history rows are written, while the existing `api_extendedfield` database trigger refreshes the affected `FacilityIndex.extended_fields` values automatically. Extended fields created from a `FacilityClaim` have no list item and are untouched, and a source whose contributor has been cleared is skipped because `ExtendedField.contributor` is NOT NULL. Saves that do not change the contributor, and saves passing an `update_fields` list without `contributor`, do no extra work. Added the `backfill_extended_field_contributors` management command to repair locations affected before this release. It re-attributes every `ExtendedField` whose contributor no longer matches the contributor of the source it was contributed through, using `bulk_update` on `contributor` only (no `updated_at` change and no new history rows), letting the same database trigger reindex the affected locations. The command is idempotent, skips sources with no contributor, and supports `--dry-run` and `--batch-size`. It is wired into `post_deployment` for this release only — see Release instructions.
...
=======
    * `backfill_extended_field_contributors` — one-time data fix (OSDEV-2159) that re-attributes every `ExtendedField` whose contributor drifted from the contributor of the source it was contributed through, which happened whenever a list's `Source.contributor` was reassigned before this release; remove from `post_deployment` after this release has been deployed everywhere.
* Run `backfill_extended_field_contributors --dry-run` against Staging (and, if possible, a Production dump) before the release to confirm the number of affected rows. Each update invokes the existing `ExtendedField` database trigger, which refreshes the affected `FacilityIndex.extended_fields` data without a separate full or targeted reindex, so no `reindex_database` or `backfill_facility_index` call is required. The command is idempotent and runs with no application downtime. Confirm timing on Staging before Production.
>>>>>>> d9cff4d8 (OSDEV-2159: List management: Extended fields retain original contributor after list source reassignment)
=======
...
>>>>>>> f4185762 (Running update on EF per batch and removing new command from post deployment command)
