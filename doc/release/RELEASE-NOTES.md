...
* [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Decoupled synthetic and infrastructure liveness from django-watchman database/cache checks. BetterStack, the ALB target-group health check, and the ECS task health check should use `GET /health-check/ping/` (returns `pong`, no Postgres/cache round-trips). `GET /health-check/` remains the deep readiness endpoint (`WATCHMAN_CHECKS`). `OriginSourceMiddleware` and `DarkVisitorsMiddleware` skip `/health-check/*`. Added CloudWatch alarms for ALB `HTTPCode_Target_5XX_Count`, ALB `TargetResponseTime`, and RDS `DatabaseConnections` (SNS via `topic…GlobalNotifications`). Documented existing RDS/cache/ALB/ECS alarms and BetterStack cutover in `doc/ops/monitoring.md`. **Post-deploy:** point BetterStack monitors at `/health-check/ping/` and confirm SNS subscriptions for the new alarms.
