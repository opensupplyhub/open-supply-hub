...
* [OSDEV-2952](https://opensupplyhub.atlassian.net/browse/OSDEV-2952) - Updated the legacy Swagger/OpenAPI documentation for `GET /api/facilities/{id}/` so the generated schema reflects the full response. Added typed schema serializers (`ClaimInfoSerializer`, `ContributorEntrySerializer`, `SectorEntrySerializer`, `ExtendedFieldsSerializer`, `OtherLocationSerializer`, `CreatedFromSerializer`, `ActivityReportEntrySerializer`, `ContributorFieldEntrySerializer`) in `facility_index_details_schema_serializers.py` and annotated the corresponding `SerializerMethodField` getters on `FacilityIndexDetailsSerializer`/`FacilityIndexSerializer` with `@swagger_serializer_method`, so previously-undocumented properties (`claim_info`, `other_locations`, `extended_fields`, `sector`, `created_from`, `is_claimed`, `activity_reports`, `contributor_fields`, and the enriched `contributors`) are now derived from code instead of a hand-maintained sample. Extended the docstring sample response accordingly and added a drift-guard test (`test_facility_index_details_schema.py`) that fails if a new method field is added without a schema annotation.
