...
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/taxonomy-config/` to expose runtime ISIC Rev 4 settings (`enabled`, `version`, `taxonomyUrl`) for the search UI; responses are cached server-side for 60 seconds and invalidated on publish or enable/disable. Added `GET /api/taxonomy/isic4/` so Django serves the active taxonomy JSON from the private files bucket only after staff publish via admin. `/web/environment.js` injects `ISIC4_TAXONOMY_ENABLED`, `ISIC4_TAXONOMY_VERSION`, and `ISIC4_TAXONOMY_URL` so the filter can gate without an extra round-trip.
