...
### Bugfix
* [OSDEV-2384](https://opensupplyhub.atlassian.net/browse/OSDEV-2384) - Fixed embedded-map XLSX downloads from `GET /api/facilities-downloads/` returning empty configured custom-field columns when list-upload headers contained mixed-case names, and empty standard extended-field columns when those values came from other contributors. Custom fields remain scoped to the embed contributor, while standard fields now match regular download aggregation.
