...
* `0225_create_isic_taxonomy_config.py` - Creates the singleton `IsicTaxonomyConfig` table and seeds v1 from the committed `isic_rev4.json`, uploading `isic_rev4.json` and `isicRev4Taxonomy.js` artifacts to the files bucket at `taxonomy/isic4/v1/` when S3 is configured.
...
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/taxonomy-config/` to expose runtime ISIC Rev 4 settings (`enabled`, `version`, presigned `bundleUrl`) for the search UI; responses are cached server-side for 60 seconds and invalidated on publish or enable/disable. `/web/environment.js` now also injects `ISIC4_TAXONOMY_ENABLED`, `ISIC4_TAXONOMY_VERSION`, and `ISIC4_TAXONOMY_BUNDLE_URL` so the filter can gate without an extra round-trip.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added an ISIC taxonomy admin page at `/admin/api/isictaxonomyconfig/` (listed under **Api → ISIC taxonomies** in the Django sidebar) for staff with `api.change_isictaxonomyconfig` permission to upload CSV/XLSX/ODS taxonomy spreadsheets (11-column ISIC Rev 4 schema), preview the parsed Section → Division → Group → Class hierarchy, atomically publish validated artifacts to versioned S3 prefixes, and enable or disable the search UI without deleting published versions. The React search UI now dynamically imports the published JS bundle (with a bundled fallback for local dev) and hides the ISIC filter and the "Match both facility type and ISIC categories" combine checkbox when the taxonomy is disabled; ISIC display on facility profiles is unchanged.
...
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Staff can upload and publish updated ISIC Rev 4 taxonomies from Django admin without a code deploy; the search UI loads the active version from the files bucket at runtime.

### Architecture/Environment changes
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added CORS rules on the files bucket so browsers can dynamically import ISIC taxonomy JS bundles via presigned URLs: MinIO setup in `scripts/reset_database` for local development and `aws_s3_bucket_cors_configuration` on the AWS files bucket in Terraform.
