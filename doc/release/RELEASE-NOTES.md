...
* [OSDEV-2424](https://opensupplyhub.atlassian.net/browse/OSDEV-2424) - Added partner-field columns to CSV and XLSX facility downloads. Every `PartnerField` that is `active=True` **and** `system_field=False` is appended to the download after the `is_closed` column, so the full layout is: 16 core columns + 35 claim columns + `is_closed` + N partner-field columns. System partner fields (e.g. `wage_indicator`, `mit_living_wage`) are excluded because they are synthesized at read time from registered providers rather than stored as contributor extended fields.
    * **Object-typed partner fields** (those declaring a JSON Schema) are flattened into one column per leaf property using `.`-joined paths (e.g. `bsci_audit.submission_date`, `amfori_compliance_status.bepi_audit.expiration_date`). Nested object schemas are walked recursively in declaration order so column order is deterministic.
    * **Primitive partner fields** become a single column named after the field itself.
    * **Multiple contributions** for the same partner field on one production location are joined with `|` per column, consistent with the existing `number_of_workers` / `parent_company` / `product_type` behavior.
    * **Embed downloads** are unaffected: partner-field columns are only appended on the default (non-embed) download serializer.
    * **Caching** — Both `FacilitiesDownloadService.get_active_partner_fields()` and `FacilityIndexDetailsSerializer` now read through a shared `get_cached_all_partner_fields()` helper that stores the full `PartnerField.objects.all()` superset under `PARTNER_FIELD_LIST_KEY` (TTL controlled by the new `PARTNER_FIELD_LIST_CACHE_TTL_SECONDS` constant). Each caller filters the cached list in memory for what it needs, so the entry is consistent regardless of which endpoint warms it first. A paginated download session still issues one query for partner fields instead of one per page, and `PartnerField.save`/`delete` hooks continue to invalidate the entry so new or deactivated partners appear in subsequent reads.
