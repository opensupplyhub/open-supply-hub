...
### Bugfix
* [OSDEV-3114](https://opensupplyhub.atlassian.net/browse/OSDEV-3114) - Fixed intermittent local stack startup failures on `docker compose up -d`. Kafka in ZooKeeper mode could crash with `NodeExistsException` when the broker container was recreated before ZooKeeper cleared the stale `/brokers/ids/1` znode — migrated local Kafka to single-node KRaft in `docker-compose.yml` and removed the `zookeeper` service. The React dev server on `http://localhost:6543/` could exit with `ERR_PACKAGE_PATH_NOT_EXPORTED` on `postcss/lib/tokenize` when its anonymous `node_modules` volume retained an outdated dependency tree; replaced it with the named `react_node_modules` volume and rebuilt dependencies. The existing [OSDEV-2995](https://opensupplyhub.atlassian.net/browse/OSDEV-2995) Yarn resolution already installs `postcss-safe-parser`'s nested PostCSS at 8.5.16. Local dev only; AWS uses MSK and static React builds.
