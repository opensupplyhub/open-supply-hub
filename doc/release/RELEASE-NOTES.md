...
* [OSDEV-2665](https://opensupplyhub.atlassian.net/browse/OSDEV-2665) - Added DMARC `_dmarc` TXT records in Route 53 to enable external compliance monitoring via `inbound.axl.net`: `opensupplyhub.org` (`p=reject`, replaces the previous `aws-accounting@opensupplyhub.org` `rua` address) and `openapparel.org` (`p=none`, adds `rua` reporting address). Records are scoped to the `Production` environment only in `dns.tf` (`count=0` for all others) so that Staging and RBA deployments do not attempt to create duplicate records in the shared hosted zone. The `_dmarc.os-hub.net` record was provisioned directly in the dev AWS account outside Terraform; a `removed { lifecycle { destroy = false } }` block ensures the next Development deployment detaches it from Terraform state without deleting the live Route 53 entry.
...
### Bugfix
* [OSDEV-2860](https://opensupplyhub.atlassian.net/browse/OSDEV-2860) - Fixed all Docker image builds failing during `pip install` with `Could not find a version that satisfies the requirement base32-crockford==0.3.0 (from versions: none)`. PyPI's simple index for `base32-crockford` (last published 2015) no longer lists any distribution files, making the package uninstallable. Removed the dependency from `src/django/requirements.txt` and `src/dedupe-hub/api/requirements.txt` and inlined the two used pieces — the Crockford base32 symbol table and integer-to-base32 encode function — directly in both `os_id.py` files.
