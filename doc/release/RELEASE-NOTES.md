...
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Enabled `pgaudit` database activity auditing on the RDS for PostgreSQL instances, resolving the Vanta SOC 2 test `aws-rds-pgaudit-enabled`. `aws_db_parameter_group.default` in `deployment/terraform/database.tf` now sets `shared_preload_libraries` to `pg_stat_statements,pgaudit` and `pgaudit.log` to `ddl,role`, both with `apply_method = "pending-reboot"` because `shared_preload_libraries` is a static PostgreSQL parameter. The values come from new `rds_shared_preload_libraries` and `rds_pgaudit_log` variables, typed and validated at plan time so an override cannot drop `pgaudit` from the library list or set an unsupported audit class. `ddl,role` audits schema changes and privilege/role grants; `read` and `write` are excluded because logging every SELECT, or every write on the ingestion path, would multiply log volume. `pg_stat_statements` is named explicitly because a custom parameter group replaces the family default outright, and omitting it would silently disable query statistics. `pgaudit` was also added to `install_db_exts`, which now collects failures and exits non-zero instead of swallowing them, so a green run is proof the extension exists. All six environments run a single-instance RDS PostgreSQL 16 database with no Aurora cluster, so the Aurora acceptance criteria do not apply. Enabling this takes a staged two-phase rollout per environment — `CREATE EXTENSION pgaudit` must land before `pgaudit.log` is set, or DDL audit records are written without object type or object name — see Release instructions and `doc/ops/database-auditing.md`. PostgreSQL logs are still not exported to CloudWatch Logs, so audit records stay in RDS-local files under RDS's default retention; centralizing them is out of scope here.
...
* Ensure that the following commands are included in the `post_deployment` command:
    * `migrate`
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Enable `pgaudit` on the RDS instance in each environment. This is a two-phase rollout and the order matters: `CREATE EXTENSION pgaudit` must land before `pgaudit.log` is set, otherwise DDL audit records are written without object type or object name. None of it is automated, since each phase needs a reboot. Both phases can run back to back in one maintenance window; the full runbook and verification queries are in `doc/ops/database-auditing.md`.
    1. Phase 1 — add a temporary `rds_pgaudit_log = "none"` override to the environment tfvars and apply Terraform. Both parameters land on the parameter group as `pending-reboot`.
    ...
    3. Run `./deployment/run_cli_task <Environment> "install_db_exts"` to create the extension. The command now exits non-zero if any extension fails, so a green run is the completion signal. Do not start phase 2 if this step fails.
    4. Phase 2 — remove the `rds_pgaudit_log` override so the environment falls back to the `ddl,role` default, apply Terraform, and reboot the instance again.
    5. Verify: `SHOW shared_preload_libraries;` (expect `rdsutils,pg_stat_statements,pgaudit`), `SHOW pgaudit.log;` (expect `ddl,role`), and `SELECT extname FROM pg_extension WHERE extname = 'pgaudit';`. Run a harmless DDL statement and confirm its audit record carries a populated object type and object name. Then re-run the Vanta test `aws-rds-pgaudit-enabled` and confirm the environment passes.
