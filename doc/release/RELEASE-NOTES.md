...
### Bugfix
* [OSDEV-2724](https://opensupplyhub.atlassian.net/browse/OSDEV-2724) - Restored the uploaded-file row numbering on facility list pages. The original OSDEV-2724 fix correctly blanked the header row number but also changed `uploadedFileRowIndexOffset` from 2 to 1, which made every displayed row number one less than the corresponding row in the contributor's uploaded file. ContriBot reports and data-team feedback emails cite file row numbers (header = row 1, first data row = 2), so moderators cross-referencing those against the list page were off by one — risking removal of the wrong rows. The offset is back to 2 (list rows now display 2..N+1, matching the file exactly); the header stays blank. The constant now carries a comment explaining why it must remain 2.
