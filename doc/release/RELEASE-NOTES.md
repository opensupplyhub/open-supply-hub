...
* [OSDEV-3459](https://opensupplyhub.atlassian.net/browse/OSDEV-3459) - The `reassert_rba_promotions` command added in [OSDEV-3438](https://opensupplyhub.atlassian.net/browse/OSDEV-3438) now runs automatically on the private instance, triggered by the nightly database sync job succeeding rather than by a clock. The sync is what reverts promotions, and it has no fixed duration — it checkpoints and resumes — so a second cron expression would either race it or idle; a new EventBridge rule matches the sync job's own `SUCCEEDED` Batch event instead. The re-assert runs as a new Batch job definition on the existing sync queue and compute environment, so no new compute is provisioned. The rule matches on job name prefix, not queue alone: both jobs share a queue, so a queue-only pattern would match the re-assert's own completion and resubmit it indefinitely. The job definition carries only what the command needs — database connection, `DJANGO_ENV`, and `INSTANCE_SOURCE`, which the command's instance guard reads — with no EFS mount and none of the source-database configuration the sync requires. It is not retried: a non-zero exit means a specific promotion could not be restored, which a repeat run reproduces rather than clears, so one failure raises one alert instead of three. Controlled by a new `reassert_rba_promotions_enabled` tfvars flag, so the job can be turned off without a code change. All resources are gated on the private-instance environment; no other environment plans a change.
* **Batch failure alerting extended to the sync queue.** The `AlertBatchFailures` rule matched only the default job queue, so failures of the nightly database sync — and, without this, of the re-assert job above — raised no alert at all. Its event pattern now also covers the sync queue on the instance that runs it. Alerts continue to go to Rollbar via the existing Lambda; no new alerting path was added. The export_csv queue is still outside the pattern and is left for separate work.
...
* The private instance needs **Deploy to AWS** with `deploy-mode` set to `terraform-plan-and-apply` to create the new re-assert job definition, rule and target (OSDEV-3459). The Batch job runs the image at `image_tag`, so the instance must be deployed on this release before the schedule can restore anything. Other environments apply only the widened `AlertBatchFailures` event pattern.
