...
* Ensure that the following commands are included in the `post_deployment` command:
    ...
    * `backfill_facility_index --fields claim_info,approved_claim,processing_type --parallel 10 --batch-size 10000` — one-time targeted backfill that recomputes indexed claim JSON without `closing_date` (OSDEV-2977) and refreshes `FacilityIndex.processing_type` after migration `0220` changes `index_processing_type()` (OSDEV-1034). The processing-type field group only touches facilities that have a `processing_type` `ExtendedField`. The worker count is set per environment via the temporary `BACKFILL_PARALLEL_BY_ENVIRONMENT` map in `post_deployment.py`; remove the call, the map, and the helper after this release has been deployed everywhere.
...
* After deployment, verify that the new Craft homepage is served at `opensupplyhub.org/` and that the existing map page renders correctly at `opensupplyhub.org/map`.
