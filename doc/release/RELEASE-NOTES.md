...
* [OSDEV-2646](https://opensupplyhub.atlassian.net/browse/OSDEV-2646) - **Deploy to AWS** now builds and pushes ECR service images in parallel (matrix job with `max-parallel: 4`) instead of sequentially in one job. A prepare step still checks immutable ECR tags and omits images that already exist for the commit SHA; production-only and test-only Dockerfiles remain env-conditional. Removed the redundant `update_services` job (`force-new-deployment`): Terraform apply already registers a new task definition revision and ECS rolls out automatically when the service's task definition changes. Expected deploy speedup: build segment duration should approach the slowest image (two waves at most) rather than the sum of all builds (~8–18 minutes on typical runs).
