...
### Code/API changes
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Implemented the ContriBot `fetch_lists` Lambda: reads the DynamoDB resume cursor (falls back to `LAST_LIST_ID` when the `__CURSOR__` item is missing or invalid), paginates `GET /api/admin-facility-lists/` with `id__gt` and `ordering=id`, validates DRF absolute `next` URLs against `OS_HUB_API_URL`, writes each newly seen list as `PENDING` in DynamoDB while skipping lists already enqueued, persists `contributor_name`, `contributor_email`, and `file_name` on each row for downstream `process_list` use, advances the cursor to the highest fetched id, and returns Map-state `{list_id}` items for Step Functions.
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Implemented the ContriBot `notify` Lambda so moderators get Slack notifications again (previously provided by the retired standalone `contribot` repository): reads the list row written by `fetch_lists` from DynamoDB, posts to the Slack incoming webhook stored in the `contribotSlackApiUrl` secret (new `lib/slack_webhook.py`, stdlib `urllib` + Secrets Manager, matching `OSHubAPI`), and records the final list status (`PROCESSED`/`FAILED` plus `finished_at`) via new `ListsRepository.get_list`/`finish_list` methods. Success messages carry the list link, contributor admin link, contributor email, and file name; when `process_list` (still a stub) later supplies `num_lines`/`num_errors`/`error_ratio`/`report_url`, the message adds the checked-report link and the error-ratio mood emoji from the original ContriBot. Failure messages (Step Functions `Catch` path) include the error cause. A Slack outage logs the unsent message and returns `notified: false` without failing the workflow. The `contribot_notify` zip now bundles the `lib` modules like `fetch_lists`. Monday board posting is not yet implemented.
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Refactored the ContriBot validation library ahead of automation work: added `ContriBot.process()` as the single entry point, removed unused SQLAlchemy/MySQL/PostgreSQL persistence from `save()`, dropped the streamlit/`stqdm` code path, and trimmed `requirements.txt` to packages still in use.
* [OSDEV-2924](https://opensupplyhub.atlassian.net/browse/OSDEV-2924) - Bumped `django` 5.2.14 → 5.2.16 (security patch releases on the 5.2 LTS line). 5.2.15 fixes CVE-2026-7666 — the SMTP email backend no longer reuses a partially-initialized connection after a failed STARTTLS handshake (production uses the Amazon SES backend, so no behavior change expected); 5.2.16 adds three low-severity cache-related fixes.
