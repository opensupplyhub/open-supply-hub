...
### Code/API changes
* [OSDEV-3435](https://opensupplyhub.atlassian.net/browse/OSDEV-3435) - `POST /api/facilities/merge/` now refuses, on the RBA private instance only, any merge where either production location was synced from public OS Hub — returning 400 and naming the OS IDs that cannot be merged. The one-way nightly sync upserts by `uuid` and never deletes, so merging a publicly-synced record is only half-applied: the merged-away facility is recreated on the next run because it still exists publicly, while the aliases and matches the merge re-pointed stay with the target, and the sync cannot repair that state. Merges between records created on the instance are unaffected, since the sync never reads rows with no public counterpart. The guard is scoped by `INSTANCE_SOURCE`, so public OS Hub behaviour is unchanged and no new configuration is required; a record with no `origin_source` is treated as synced, so the check fails closed. It is enforced server-side before any writes, which covers the dashboard merge flow through the same endpoint. New shared helper `api/helpers/rba_instance.py`.
