...
### Code/API changes
* [OSDEV-3438](https://opensupplyhub.atlassian.net/browse/OSDEV-3438) - New `reassert_rba_promotions` management command restores promotions that the one-way sync from OS Hub reverts on the RBA private instance. The sync overwrites every synced field of a shared facility, including `created_from`, so a promotion made on the private instance is silently undone whenever the public record next changes — while the contribution itself survives, because its source, list item, and match are rows created on that instance and the sync only upserts rows that exist publicly. The command therefore derives the reverted set by query rather than from a journal: the newest active, confirmed, instance-origin match per facility whose list item carries a `PROMOTE_MATCH` processing result, excluding facilities already created from it. Requiring that recorded evidence of a prior promotion is what separates a reverted promotion from the ordinary case of a contribution that was matched to a facility and never promoted — the latter is the majority of instance-origin matches, and selecting them would rewrite the canonical fields of facilities nobody ever promoted. Each restore is a single promote mirroring the promote endpoint: canonical fields, `created_from`, a `PROMOTE_MATCH` processing result, and a `Promoted … over … (re-asserted after sync)` change reason that the facility history parser still recognises. Considering only the newest promoted match means a run can never demote a facility to an older contribution nor overrule a moderator who deliberately promoted an earlier one, and facilities already promoted are not selected, so repeat runs make no writes. Items with no geocoded point are skipped rather than failing on save. Supports `--dry-run` and `--limit`, and refuses to run outside the RBA instance unless `--allow-non-rba-instance` is passed. Nothing schedules it yet — it is run manually, and only affects the RBA environment.
