...
* `0220_remove_closing_date_from_claim_index_functions.py` - Updates `index_claim_info()` and `index_approved_claim()` to omit `closing_date` from indexed claim JSON.
* `0221_remove_closing_date_from_facility_claim.py` - Drops `closing_date` from `api_facilityclaim` and `api_historicalfacilityclaim`.
...
* [OSDEV-2977](https://opensupplyhub.atlassian.net/browse/OSDEV-2977) - **Release step required:** run the `[Release] Deploy` workflow with `clear-opensearch-target` set to `production-locations` (index clear + full Logstash re-sync) so stale `closed_at` values are removed from OpenSearch. This is the standard required index refresh for the release. The `production-locations` index is deleted before Logstash starts repopulating it, so v1 production-location search results will be incomplete and detail requests may return 404 until the rebuild finishes (historically ~4 hours).
