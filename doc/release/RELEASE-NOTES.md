...
* [OSDEV-2424](https://opensupplyhub.atlassian.net/browse/OSDEV-2424) - Added partner-field columns to CSV and XLSX facility downloads. Every active, non-system `PartnerField` is appended to the download after the `is_closed` column, so the full layout is: 16 core columns + 35 claim columns + `is_closed` + N partner-field columns.
    * **Object-typed partner fields** (those declaring a JSON Schema) are flattened into one column per leaf property using `.`-joined paths. Nested object schemas are walked recursively in declaration order so column order is deterministic.
    ...
    * **Caching** — `FacilitiesDownloadService.get_active_partner_fields()` reads the partner-field list from the shared `PARTNER_FIELD_LIST_KEY` cache (TTL controlled by the new `PARTNER_FIELD_LIST_CACHE_TTL_SECONDS` constant), so a paginated download session issues one query for partner fields instead of one per page. The same cache key is reused by the production-location details serializer, and `PartnerField.save`/`delete` hooks continue to invalidate it so new partners appear in subsequent exports.
