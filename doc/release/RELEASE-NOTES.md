...
* [OSDEV-2644](https://opensupplyhub.atlassian.net/browse/OSDEV-2644) - **Code Quality** now skips jobs whose trees did not change in the PR, via `dorny/paths-filter` and job-level `if` (not workflow `paths`, so required checks still report). Docs, `deployment/`, `src/batch`, `src/anon-tools`, `src/kafka-tools`, `src/maintenance-page`, and other unmapped paths skip the heavy suites. Application changes still run the matching checks: React (Jest/coverage and ESLint/Prettier), Django `api`, countries, contricleaner, Dedupe Hub, ContriBot, and integration (Python API tests plus Playwright against the live stack). Countries and contricleaner run only for their app tree plus Django image/deps (`Dockerfile`, `requirements.txt`, `docker-compose.yml`), not `oar/**` or `manage.py`. Flake8 skips when the only Django changes are files it already `--exclude`s (`manage.py`, `settings.py`, `api/migrations`). Changing `.github/workflows/code_quality.yml` is the always-run allowlist and forces every job.
