...
* Ensure that the following commands are included in the `post_deployment` command:
    ...
    * `backfill_facility_index --fields claim_info,approved_claim --parallel 10 --batch-size 10000` — one-time backfill (OSDEV-2977) that recomputes indexed claim JSON without `closing_date`; remove from `post_deployment` after this release has been deployed everywhere.
