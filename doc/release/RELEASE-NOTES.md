...
### Bugfix
* [OSDEV-3114](https://opensupplyhub.atlassian.net/browse/OSDEV-3114) - Fixed intermittent local stack startup failures on `docker compose up -d`. Kafka in ZooKeeper mode could crash with `NodeExistsException` when the broker container was recreated before ZooKeeper cleared the stale `/brokers/ids/1` znode — migrated local Kafka to single-node KRaft in `docker-compose.yml` and removed the `zookeeper` service. The React dev server on `http://localhost:6543/` could exit with `ERR_PACKAGE_PATH_NOT_EXPORTED` on `postcss/lib/tokenize` — the Yarn resolution from [OSDEV-2995](https://opensupplyhub.atlassian.net/browse/OSDEV-2995) used `**/postcss-safe-parser/postcss`, which Yarn v1 does not apply; corrected to `postcss-safe-parser/postcss: 8.5.16`, replaced the anonymous `node_modules` volume with named `react_node_modules`, and rebuilt the React image. Local dev only; AWS uses MSK and static React builds.
