...
* `0229_create_facility_processing_value_index.py` - Creates `api_facility_processing_value`, a precomputed index of distinct facility and processing type values with facility counts, plus triggers that keep it synchronized with `FacilityIndex`. Empty values and common contributor placeholders such as `null`, `none`, and `unknown` are excluded.
