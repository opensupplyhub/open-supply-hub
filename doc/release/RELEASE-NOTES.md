...
* `0224_add_lei_mapping.py` - Creates the `api_leimapping` table (OSDEV-3095). Plain `CreateModel`, no data migration.

#### Schema changes
* [OSDEV-3095](https://opensupplyhub.atlassian.net/browse/OSDEV-3095) - New `LeiMapping` model (`api_leimapping`): the internal audit ledger for GLEIF OS Hub→LEI mappings — `os_id` (unique, canonical), `lei`, `match_type` (`facility_name`/`parent_company`), `matched_name`, `score`, `mapping_file_date`, `status` (`active`/`removed`/`denylisted`), timestamps. Never exposed publicly; answers "why does this facility have this LEI?" and drives idempotent ingestion.

### Code/API changes
* [OSDEV-3095](https://opensupplyhub.atlassian.net/browse/OSDEV-3095) - Added the `ingest_lei_mappings` management command (GLEIF/GODIN pilot), **create-only in this release**. It reads an OS Hub→LEI mapping CSV in OS Hub's normalized internal format (`--file`, `--dry-run`, `--batch-size`; the GLEIF contributor account is resolved by its canonical admin email, and converting GLEIF's delivery file into this format is a separate step, OSDEV-3096) and materializes each *new* mapping as a real contribution chain attributed to the GLEIF contributor — one `FacilityList` + `Source` (`create=False`, so Dedupe Hub/geocoding are never triggered) per file, and per facility a `FacilityListItem` (`CONFIRMED_MATCH`) + active `FacilityMatch` (`CONFIRMED`) + `lei_id` `ExtendedField` (`{'raw_value': <LEI>, 'match_type': ...}`), plus a `LeiMapping` ledger row. Re-runs are no-ops; OS IDs already in the ledger (changed, removed, or denylisted) are skipped and counted — update/removal/denylist diff semantics arrive with the monthly-process work (OSDEV-3097). Merged OS IDs resolve via `FacilityAlias` to the canonical facility; unknown OS IDs, invalid LEIs, and duplicate targets are skipped and counted. Website display of LEIs remains gated by the existing `show_additional_identifiers` switch (OFF in production); note the switch does not gate API responses — that decision is tracked on OSDEV-3096 before any production import.
