...
* [OSDEV-2424](https://opensupplyhub.atlassian.net/browse/OSDEV-2424) - Added partner-field columns to CSV and XLSX facility downloads. Every `PartnerField` with `active=True` and `system_field=False` is appended after the `is_closed` column, followed by two download-only MIT Living Wage columns.
