...
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Enabled `pgaudit` database activity auditing on the RDS for PostgreSQL instances, resolving the Vanta SOC 2 test `aws-rds-pgaudit-enabled`. `aws_db_parameter_group.default` in `deployment/terraform/database.tf` now sets `shared_preload_libraries` to `pg_stat_statements,pgaudit` and `pgaudit.log` to `ddl,role`, both with `apply_method = "pending-reboot"` because `shared_preload_libraries` is a static PostgreSQL parameter. The values come from new `rds_shared_preload_libraries` and `rds_pgaudit_log` variables and are left at their defaults in every environment. `ddl,role` audits schema changes and privilege/role grants; `read` and `write` are excluded because logging every SELECT, or every write on the ingestion path, would multiply log volume. `pg_stat_statements` is named explicitly because a custom parameter group replaces the family default outright, and omitting it would silently disable query statistics. `pgaudit` was also added to the idempotent `install_db_exts` management command, making `CREATE EXTENSION pgaudit` a repeatable step. All six environments run a single-instance RDS PostgreSQL 16 database with no Aurora cluster, so the Aurora acceptance criteria do not apply. Auditing starts only after a manual RDS reboot per environment — see Release instructions and `doc/ops/database-auditing.md`. PostgreSQL logs are still not exported to CloudWatch Logs, so audit records stay in RDS-local files under RDS's default retention; centralizing them is out of scope here.
