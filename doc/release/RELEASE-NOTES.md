...
* Ensure that the following commands are included in the `post_deployment` command:
    * `migrate`
    * `reindex_database`
* Run `[Release] Deploy` pipeline for the target environment with the flag 'Clear OpenSearch indexes' set to true - to refresh the index mappings for the `production-locations` and `moderation-events` indexes after fixing the process of clearing the custom OpenSearch indexes.
