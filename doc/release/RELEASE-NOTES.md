...
* [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Made `GET /health-check/` app liveness only (HTTP 200, body `ok`) — no django-watchman Postgres/cache checks. Removed `django-watchman`. BetterStack, ALB, and ECS continue to use `/health-check/`; DB degradation is covered by CloudWatch RDS alarms including `DatabaseConnections` via `terraform-aws-postgresql-rds` ≥ 3.3.0. Per-env `rds_database_connections_alarm_threshold` is set to ~80% of each instance’s RDS default `max_connections` (`LEAST(DBInstanceClassMemory/9531392, 5000)`): Dev 90, Staging 720, Test 2880, Preprod/Production/RBA 4000. The 80% mark pages while ~20% headroom remains, so on-call is alerted before new clients fail with “too many connections” under connection pressure. CloudWatch alarms on `aws_sns_topic.global` (`topic…GlobalNotifications`) go to Slack via AWS Chatbot using required non-empty `aws_chatbot_slack_team_id` and `aws_chatbot_slack_channel_id` in private `ci-deployment` tfvars; Chatbot uses `ReadOnlyAccess` guardrails and `CloudWatchReadOnlyAccess` on its role. `OriginSourceMiddleware` and `DarkVisitorsMiddleware` skip `/health-check/`. See `doc/ops/monitoring.md`. **Post-deploy:** if BetterStack still expects the old watchman JSON body, update it to accept `ok`; merge/tag the RDS module PR then switch the module `ref` to `3.3.0`; authorize Slack once per AWS account in Chatbot, invite the app to the channel, fill the Slack IDs in `ci-deployment`, apply, and confirm the SNS topic has a Chatbot subscription and a test ALARM reaches Slack.
