...
* [OSDEV-3350](https://opensupplyhub.atlassian.net/browse/OSDEV-3350) - **This release activates the AWS ContriBot instance in production.** Production was the only environment missing the two non-secret ContriBot variables, so Terraform defaults applied: `contribot_last_list_id` (default `"NaN"`, which made the fetch Lambda raise `ValueError: Invalid LAST_LIST_ID` on every scheduled run before it reached the API) and `contribot_google_drive_shared_directory_id` (default empty, which would have failed report upload). Both are now set and the five `oshub/production/contribot-*` secrets are already populated, so the scheduled fetch starts processing production lists as soon as this release is applied. Two things to check before deploying: `contribot_last_list_id` is `9636`, so the instance starts at list 9637 and does not reprocess lists the legacy instance already handled; and `contribot_monday_board_id` points at the **live approval-queue board**, so ContriBot items will appear alongside real moderation work unless it is repointed at a separate parallel-run board first. The legacy EC2 instance keeps running in parallel; decommissioning it is gated on verified production health and tracked separately.
