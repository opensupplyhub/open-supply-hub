...
* The `backfill_facility_index --fields processing_type,sector` reindex runs with no application downtime; monitor RDS CPU and connections on the first run after deploy and reduce `--parallel` if needed. Confirm timing on Staging before Production.
