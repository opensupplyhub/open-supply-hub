...
* [OSDEV-2949](https://opensupplyhub.atlassian.net/browse/OSDEV-2949) - Run `strip_unknown_partner_field_keys` once after `migrate` to purge already-stored undeclared partner-field keys (e.g. the reported `internal_ID`) from `ExtendedField.value.raw_values` and reindex the affected facilities. Preview with `--dry-run` first. The API output filter hides these keys immediately regardless, but this command removes the sensitive values at rest and from the search index. Remove from `post_deployment` after this release.
