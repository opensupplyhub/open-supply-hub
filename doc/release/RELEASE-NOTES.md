...
* Fixed local and CI database image builds against GDAL 3.13. PostGIS 3.4.2 raster compilation referenced GDAL's removed `MIN` macro, so the build now applies the upstream-compatible `Min` replacement before compilation. This does not change the database schema or runtime PostGIS version.
