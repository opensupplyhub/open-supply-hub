...
* [OSDEV-2929](https://opensupplyhub.atlassian.net/browse/OSDEV-2929) - First stage of transitioning deployment secrets from the private `ci-deployment` tfvars repo to AWS Secrets Manager (CLI-owned `oshub/<env>/<purpose>` secrets). Public `deployment/environments/terraform-*.tfvars` hold SM secret names plus non-sensitive deployment config; **Deploy to AWS** and **destroy** no longer check out or merge `ci-deployment`. Terraform resolves secret values at plan/apply; ECS/Batch use `valueFrom` for core runtime secrets (RDS, Django key, email addresses, Google/OAR keys) while other SM-backed values are still injected via the task `environment` block (visible in Terraform state and the ECS task definition JSON — acceptable for this first stage).
