...
* [OSDEV-2384](https://opensupplyhub.atlassian.net/browse/OSDEV-2384) - Fixed embedded-map XLSX downloads from `GET /api/facilities-downloads/` returning empty configured custom-field columns when the list-upload header used mixed-case names such as `program_1_Name`. The header was lowercased before the column lookup, so any non-lowercase configured field resolved to no column and exported blank. Custom fields are now matched by their original header position, preferring an exact-case match and falling back to a case-insensitive one. Embed downloads now export only the visible embed-config columns, in the same order as the embedded profile, using the embed contributor's own data with a single top-ranked value per extended field. Columns that are hidden in the embed configuration (for example `number_of_workers` when `visible=false`) are no longer appended to the export.
