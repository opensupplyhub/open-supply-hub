...
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Phase 1 of enabling `pgaudit` database activity auditing on the RDS for PostgreSQL instances, towards the Vanta SOC 2 test `aws-rds-pgaudit-enabled`. `aws_db_parameter_group.default` in `deployment/terraform/database.tf` now sets `shared_preload_libraries` to `pg_stat_statements,pgaudit` and declares `pgaudit.log`, both with `apply_method = "pending-reboot"` because `shared_preload_libraries` is a static PostgreSQL parameter. The values come from new `rds_shared_preload_libraries` and `rds_pgaudit_log` variables, typed and validated at plan time so an override cannot drop `pgaudit` or `pg_stat_statements` from the library list or set an unsupported audit class. `rds_pgaudit_log` ships as `none`: pgaudit only records object type and object name for DDL once `CREATE EXTENSION pgaudit` has installed its event triggers, and the extension cannot be created until the library is loaded at server start — so auditing has to stay off across that first reboot, or DDL records name no object. Making `none` the committed default means a Terraform apply cannot get ahead of the extension. [OSDEV-3236](https://opensupplyhub.atlassian.net/browse/OSDEV-3236) is phase 2 and flips the default to `ddl,role`; the Vanta test still fails until it ships. `pg_stat_statements` is named explicitly because a custom parameter group replaces the family default outright, and omitting it would silently disable query statistics. `pgaudit` was also added to `install_db_exts`, which now collects failures and exits non-zero instead of swallowing them, so a green run is proof the extension exists. All six environments run a single-instance RDS PostgreSQL 16 database with no Aurora cluster, so the Aurora acceptance criteria do not apply. PostgreSQL logs are still not exported to CloudWatch Logs, so audit records will stay in RDS-local files under RDS's default retention; centralizing them is out of scope here. See `doc/ops/database-auditing.md`.
...
* Ensure that the following commands are included in the `post_deployment` command:
    * `migrate`
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) - Phase 1 of the pgaudit rollout, per environment. This release only loads the library and creates the extension; auditing is switched on separately by [OSDEV-3236](https://opensupplyhub.atlassian.net/browse/OSDEV-3236), which must not ship to an environment until these steps have completed there. Full runbook in `doc/ops/database-auditing.md`.
    1. Apply Terraform. `shared_preload_libraries` and `pgaudit.log` land on the parameter group as `pending-reboot`, with `pgaudit.log` at `none`.
    2. Reboot the instance: `aws rds reboot-db-instance --db-instance-identifier opensupplyhub-enc-<env>`. `shared_preload_libraries` is a static parameter, and RDS does not apply it on its own — not even during the maintenance window. Every environment runs `rds_multi_az = false`, so this is a hard restart with roughly 30-120 seconds of database downtime rather than a failover. Schedule it for Production.
    3. Run `./deployment/run_cli_task <Environment> "install_db_exts"` to create the extension. The command now exits non-zero if any extension fails, so a green run is the completion signal.
    4. Confirm before marking the environment done: `SHOW shared_preload_libraries;` (expect `rdsutils,pg_stat_statements,pgaudit`), `SHOW pgaudit.log;` (expect `none`), and `SELECT extname FROM pg_extension WHERE extname = 'pgaudit';`.
* Run the pgaudit sequence on Development and Staging before Production. The Vanta test `aws-rds-pgaudit-enabled` still fails after this release; it passes once OSDEV-3236 ships.
