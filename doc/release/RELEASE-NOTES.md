...
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Implemented the ContriBot `fetch_lists` Lambda (replacing the 2.28.0 stub): paginates `GET /api/admin-facility-lists/` using a DynamoDB `__CURSOR__` watermark (with `LAST_LIST_ID` fallback), enqueues new lists as `PENDING` with contributor metadata, and feeds Step Functions Map state with `{list_id}` items. Shared modules under `src/contribot/lib/` (`lists_repository.py`, `os_hub_api.py`) bundle via an updated Makefile. Terraform sets env-specific `OS_HUB_API_URL`, adds `contribot_last_list_id`, and raises ContriBot Lambda timeouts to 900 seconds.
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Implemented the ContriBot `notify` Lambda so moderators get Slack notifications again (previously provided by the retired standalone `contribot` repository): reads the list row written by `fetch_lists` from DynamoDB, posts to the Slack incoming webhook stored in the `contribotSlackApiUrl` secret (new `lib/slack_webhook.py`, stdlib `urllib` + Secrets Manager, matching `OSHubAPI`), and records the final list status (`PROCESSED`/`FAILED` plus `finished_at`) via `ListsRepository.get_list`/`update_list`. Success messages carry the list link, contributor admin link, contributor email, and file name; when `process_list` supplies `num_lines`/`num_errors`/`error_ratio`/`report_url`, the message adds the checked-report link and the error-ratio mood emoji from the original ContriBot. After a successful `process_list`, `notify` also creates a Monday approval-queue item (`lib/monday.py`) with list name, contributor name and id, processed-report Drive link, OS Hub list link, and list size (`num_lines`); column IDs are resolved from the board by title. The Step Functions `Catch` / failure path posts Slack only (no Monday item). A Slack outage is logged and does not fail the workflow (`notified: false`). A Monday failure is logged and re-raised. The `contribot_notify` zip bundles the `lib` modules like `fetch_lists`.
