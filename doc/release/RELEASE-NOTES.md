...
### Code/API changes
* [OSDEV-3404](https://opensupplyhub.atlassian.net/browse/OSDEV-3404) - The claim form's Business step can now let a claimant edit the Company Name and Company Address fields, behind the new inactive `enable_claim_name_address_edit` waffle switch. When the switch is on, both fields are prefilled from the production location, required and non-empty (200 characters max), and submitted with the claim as `facility_name_english` and `facility_address` (stored on the claim, see OSDEV-3405 below). A warning under the fields tells claimants that the entered name and address will be shown on the production location page once the claim is approved, that they must match the document or web page submitted for verification, and points them to the Single Location Contribution form when the location has moved and needs a new OS ID. The document-upload note also reads "as entered above" instead of "as listed on Open Supply Hub" while the switch is on. With the switch off the fields stay read-only and nothing changes.
* [OSDEV-3405](https://opensupplyhub.atlassian.net/browse/OSDEV-3405) - Store the claim form's name and address on the claim. `POST /api/facilities/{os_id}/claim/` and the pending-claim edit endpoint `PATCH /api/facility-claims/{id}/pending/` now accept optional `facility_name_english` and `facility_address` (200 characters max; blank is stored as NULL). A value is rejected when ContriCleaner's `clean()` helper reduces it to nothing, which means it is made up only of whitespace and the characters that helper strips: newlines, `-`, `/`, `'`, `,`, `:` and surrounding quotes. Other punctuation is not stripped by that helper, so a value such as `...` or `!!!` is accepted here exactly as it would be in a list upload. The claimant-facing pending read model and the moderator details endpoint `GET /api/facility-claims/{id}/` now return both keys on every claim; they are `null` on claims that were created without them, which includes every claim submitted while the `enable_claim_name_address_edit` switch is off. The stored values are unchanged for those claims: the form sends the fields blank when the switch is off, blank normalizes to NULL, and NULL is also what Django's default for these nullable columns wrote before this change. Promotion on the production location page already keys off these two claim fields (the NAME extended field of the approved claim and `approved_claim.facility_address`), so once the claim is approved the claimant's edited name and address are promoted with no further change. Recording them as a contribution in the location's submission history is OSDEV-3510.
* [OSDEV-3510](https://opensupplyhub.atlassian.net/browse/OSDEV-3510) - Record a claimant's name and address as a contribution (builds on OSDEV-3405). While the `enable_claim_name_address_edit` switch is on (the same switch that makes the form fields editable, so the feature ships dark as one unit; nothing is recorded or backfilled while it is off), when a moderator approves a claim that asserts a name or address, `POST /api/facility-claims/{id}/approve/` records the values as a contribution from the claimant: a `ModerationEvent` with `request_type=CLAIM` linked to the claim is created already `APPROVED` (never pending, so it does not enter the moderation queue or the external auto-approval automation) and processed through the same approval path as an SLC contribution to an existing location, producing a `SINGLE` `Source`, a `CONFIRMED_MATCH` `FacilityListItem` and a `CONFIRMED` `FacilityMatch` under the claimant's contributor. The claim fields keep driving promotion on the production location page as before; the contribution adds the submission history behind them, lists the claimant among the location's contributors and is not subject to SLC anonymization: a CLAIM event carries no source type, and the anonymization rule applies only to SLC-sourced events. A value the claim does not assert is backfilled from the production location (recorded in `backfilled_fields`), and the item carries the claim's sectors when the `Sector` table knows them. The same contribution is recorded again each time the claimant changes the name or address through `PUT /api/facility-claims/{id}/claimed/` (append-only history; earlier contributions stay active). Approval or edit fails and rolls back if the contribution cannot be recorded, rather than going live with missing history.
* [OSDEV-3406](https://opensupplyhub.atlassian.net/browse/OSDEV-3406) - The production location pin follows a claimed address (builds on the OSDEV-3510 contribution recording), behind the new inactive `enable_claim_address_pin_move` waffle switch; while it is off the contribution is placed at the current pin and nothing is geocoded. When the switch is on and an approved claim's address differs from the location's (compared case- and punctuation-insensitively) and the claimant has not already placed a pin, approval geocodes the address and moves `Facility.location` (recorded in facility history with a `FacilityClaim (<id>)` change reason) and sets `FacilityClaim.facility_location`. The pin is left where it is, and an internal `FacilityClaimReviewNote` explains why, when the geocoder returns no result or errors, or when the result is Google `APPROXIMATE` (a locality or region centroid); in either case the contribution's list item still records what the address geocoded to, so a moderator can promote it later. A geocoder failure never fails the approval. An unchanged address is never re-geocoded, so hand-placed pins survive claims that only confirm the existing address. Revoking a claim leaves the pin and the contributions in place, matching the existing claimed-details behavior.
* [OSDEV-3455](https://opensupplyhub.atlassian.net/browse/OSDEV-3455) - The claimed-facility details form (My Account > My Facilities > edit) now lets a claimant update the location's name and address after approval, completing the OSDEV-3404/3405 claim form fields. While the `enable_claim_name_address_edit` switch is on, the Facility Details section gains Facility name (English) and Facility address fields (200 characters max; a value that is only punctuation is flagged before submit) and the same warning note as the claim form, extended to say that a blank field keeps the name or address currently listed on Open Supply Hub. With the switch off the form is unchanged. Saving a changed value records it as a contribution through the existing claimed-details `PUT /api/facility-claims/{id}/claimed/` path (OSDEV-3510). The form's address geocoding now runs only for an address the claimant actually changed and only while `enable_claim_address_pin_move` is on, since the PUT propagates the geocoded point to the production location pin; previously every save of a claim holding an address but no pin re-geocoded the address and moved the pin, which the OSDEV-3405 claim form fields had started to trigger regardless of the switch. The `claimed` PUT itself now validates `facility_name_english` and `facility_address` with the claim submission rules: stripped, blank stored as NULL, 200 characters max, and a value that is only punctuation or whitespace rejected with a 400 keyed by field (such a value previously failed inside the contribution recording with a 500, and an over-long value with a database error). Clearing the address also clears the claim's pin, as before, now keyed on the normalized value so a whitespace-only address behaves like a blank one.
...
* `0237_add_facility_candidate_fields.py` - Adds five candidate columns (`is_candidate`, `polygon`, `confidence`, `external_id`, `source`) to `api_facility` and `api_historicalfacility`. Builds three **partial** indexes `CONCURRENTLY` on `api_facility` only (candidate filter, footprint GiST, and the unique index enforcing the conditional `(source, external_id)` constraint) — each stores only candidate/sourced rows, so they start near-empty and grow with candidate count rather than table size; `api_historicalfacility` gets no new indexes. Also attaches `api_facility_source_requires_external_id` (CHECK: a row with a non-empty `source` must carry an `external_id`, so sourced rows always have an idempotency key). Non-atomic migration. Index builds avoid write blocking entirely; the `ALTER TABLE` statements take brief `ACCESS EXCLUSIVE` locks guarded by a 5s `lock_timeout` — if `migrate` fails with a lock timeout during deployment, rerun it (every statement is idempotent). See OSDEV-3242.
...
* [OSDEV-3370](https://opensupplyhub.atlassian.net/browse/OSDEV-3370) - `FacilityClaimAttachments.claim` is now `CASCADE` (with a `post_delete` signal removing the stored file from S3), and new attachment uploads get opaque UUID object keys under `claim_attachments/` — the old key format embedded the contributor name, leaking PII into S3 access logs. The uploader's filename stays in `file_name`.
* [OSDEV-3371](https://opensupplyhub.atlassian.net/browse/OSDEV-3371) - `FacilityClaim.claimant_updated_at` records when the claimant last changed the claim (fields or documents). Stamped only by claimant edits — moderator actions bump `updated_at` but not this — so the claims queue can surface claims edited after review started.

### Code/API changes
* [OSDEV-3355](https://opensupplyhub.atlassian.net/browse/OSDEV-3355) - Scaffolding for the claims moderation dashboard v2: a new `/dashboard/claims-v2` route (dark behind the inactive `enable_claims_v2_dashboard` switch, superuser-gated like the rest of the dashboard) rendering a skeleton queue from the real claims API, plus the reusable plumbing the full Queue view (OSDEV-3356) builds on — note-timeline stage derivation (new / awaiting claimant / reply overdue, mirroring the automation pipeline's 15-business-day window), claimant-message templates, and Claims Tracker Jira deep-links. No user-visible change while the switch is off.
* [OSDEV-3356](https://opensupplyhub.atlassian.net/browse/OSDEV-3356) - The full claims moderation dashboard v2 **Queue workspace** on the OSDEV-3355 scaffold (still dark behind the inactive `enable_claims_v2_dashboard` switch, superuser-gated): a stage-grouped, collapsible queue rail with search/region filter/sort and J/K keyboard navigation; a claim workspace with profile anchor, claimant-submitted details, and a five-row verification panel showing tier-1 score chips from the automated review (advisory amber below threshold, never red) plus per-criterion document extracts with jump-to-source once [OSDEV-3458](https://opensupplyhub.atlassian.net/browse/OSDEV-3458) ships; an evidence viewer (inline images, PDFs in a new tab, English/original text tabs) beside a template-driven claimant composer with ★ suggested-draft prefill; approve/deny dialogs where the deny reason is required and clearly marked as emailed to the claimant verbatim, with an optional never-sent internal note; and a full-width activity timeline with an internal note box. The claims list endpoint (`GET /api/facility-claims/`) additionally returns `notes_meta` (note type + timestamp pairs, no note bodies) so the queue derives stages in one request — additive, existing consumers unaffected. The claims-v2 route renders full-width outside the shared dashboard column; all other dashboard pages are unchanged. Attachment URLs already resolve through the OSDEV-2278 authorization-checked download endpoint when the presigned URL field is absent, so this dashboard is compatible with that PR in either merge order.
* [OSDEV-3424](https://opensupplyhub.atlassian.net/browse/OSDEV-3424) - The moderation-event approval endpoints now report the records they created. `PATCH /api/v1/moderation-events/{id}/production-locations/{os_id}/` and `POST /api/v1/moderation-events/{id}/production-locations/` return `item_id` and `match_id` alongside the existing `os_id`. The change is additive, so existing consumers reading `os_id` are unaffected. Previously a client that needed to act on the new contribution — promoting it to the location's primary values, for example — had to call `GET /api/facilities/{os_id}/split/` afterwards and infer which match was the new one by taking its own most recent, which is an extra call reconstructing a fact the server already had and is only reliable while nothing else is writing to that location. Note that the returned `match_id` is only *promotable* from the update (`PATCH`) flow: a newly created production location is created from its own list item, and `POST /api/facilities/{os_id}/promote/` rejects that case, so a client should branch on the flow rather than promoting unconditionally. `FacilityMatch` creation in the shared approval template now returns the created record, surfaced through a `created_facility_match` property.
* [OSDEV-3435](https://opensupplyhub.atlassian.net/browse/OSDEV-3435) - `POST /api/facilities/merge/` now refuses, on the RBA private instance only, a merge that would merge *away* a production location synced from public OS Hub — returning 400 with an explanation naming the record and pointing at merging the other direction instead. The one-way nightly sync upserts by `uuid` and never deletes, so a merge that removes a publicly-synced record locally is only half-applied: that record is recreated on the next run because it still exists publicly, while the aliases and matches the merge re-pointed stay with the target, and the sync cannot repair the result. Only the merged-away record carries that risk — the target survives the merge and none of its synced fields are written — so absorbing a locally-created duplicate *into* a synced record still works, which is the ordinary cleanup case on the instance. The guard is scoped by `INSTANCE_SOURCE` (normalized, with an unrecognized value treated as public OS Hub and logged), so public OS Hub behaviour is unchanged and no new configuration is required; a record with no `origin_source` cannot be shown to be local and is refused, so the check fails closed. Enforced server-side before any writes, and the dashboard now displays the returned explanation rather than a generic failure. New shared helper `api/helpers/rba_instance.py`.
* [OSDEV-3445](https://opensupplyhub.atlassian.net/browse/OSDEV-3445) - The SLC (Single Location Contribution) duplicate-submission check ([OSDEV-2980](https://opensupplyhub.atlassian.net/browse/OSDEV-2980)) and AI data-quality check ([OSDEV-3118](https://opensupplyhub.atlassian.net/browse/OSDEV-3118)) now also run for `UPDATE` submissions (additional information for an existing location), not only new-location `CREATE` submissions. `PATCH /api/v1/production-locations/{os_id}/` accepts the same `?duplicate_override=true` and `?ignore_warnings=true` query parameters as `POST` (only the literal values `true`/`false`; anything else returns `400`), returns the same `409` responses (`duplicate_of` / `warnings`), and the SLC update form shows the same confirmation dialogs and resubmits with the accumulated overrides. The duplicate check uses the same fuzzy name/address/country match for both request types and compares them against each other: an `UPDATE` is matched against the contributor's non-rejected SLC `CREATE` and `UPDATE` events from the last 30 minutes (and a `CREATE` is now matched against recent `UPDATE`s too), rather than on the target `os_id`, because a moderator may attach an update to a different production location than the one the contributor picked, so the submitted name/address is the more reliable identity of the submission. The `duplicate_of` object gains an `os_id` field (the matched event's facility, `null` when the match is a `CREATE`). API-sourced `PATCH` requests are still never checked, and the quality check for updates sits behind the same `slc_submission_quality_check` waffle switch.
* [OSDEV-3438](https://opensupplyhub.atlassian.net/browse/OSDEV-3438) - New `reassert_rba_promotions` management command restores promotions that the one-way sync from OS Hub reverts on the RBA private instance. The sync overwrites every synced field of a shared facility, including `created_from`, so a promotion made on the private instance is silently undone whenever the public record next changes — while the contribution itself survives, because its source, list item, and match are rows created on that instance and the sync only upserts rows that exist publicly. The command therefore derives the reverted set by query rather than from a journal, reading the facility's own history: a facility is out of date when its current `created_from` is not the list item its most recent `Promoted …` history entry names. History is the right source rather than a `PROMOTE_MATCH` processing result on the item, for two reasons — the facility delete flow appends that marker to *every* other matched item, so it does not identify the promoted one; and only history orders a promotion against everything else that has happened to the facility, so a moderator who later promotes a different contribution (including a public one, which is how a promotion gets undone) is authoritative and the facility is left alone instead of being reversed on the next run. Each restore is a single promote mirroring the promote endpoint: canonical fields, `created_from`, a `PROMOTE_MATCH` processing result, and a `Promoted … over … (re-asserted after sync)` change reason that the facility history parser still recognises — and which makes the re-assertion itself the facility's latest promotion, so repeat runs make no writes. The facility and list item rows are locked and re-read inside the transaction, and the facility is saved with `update_fields`, so a concurrent write — to an unrelated facility field, or to the item’s `processing_results` — is not overwritten from a stale copy. Items with no geocoded point are skipped rather than failing on save, and an item that has become another facility's `created_from` is reported as a named conflict rather than a swallowed `IntegrityError`. Supports `--dry-run` and `--limit`, and refuses to run outside the RBA instance unless `--allow-non-rba-instance` is passed. Nothing schedules it yet — it is run manually, and the RBA instance is its intended target; `--allow-non-rba-instance` permits an explicit run elsewhere.
* [OSDEV-3440](https://opensupplyhub.atlassian.net/browse/OSDEV-3440) - Added `scripts/rba_address_batch/`, a standalone tool a private-instance administrator can run themselves to bulk-apply address corrections from a CSV and promote each one to the location's primary address. Submitting an address does not change what a location displays — a contribution has to be promoted first — which is four operations per record through the interface. The tool performs the submit, approve, discover and promote calls per row, journals each step so an interrupted run skips finished records and picks a part-finished one up where it stopped, rather than repeating submissions or promotions that already landed, and writes a verification report for a human to review before the batch is treated as done. Read-only `--dry-run` mode confirms the token is a superuser (approve and promote both require one) and that every location exists. Optional `lat`/`lng` columns skip server-side geocoding, which is the most common cause of rejected rows. It refuses to run against public OS Hub, where bulk-promoting one contributor's submissions would override other contributors' data. No platform code changes; configuration is entirely by environment variable. Requires [OSDEV-3438](https://opensupplyhub.atlassian.net/browse/OSDEV-3438), in this same release, to be deployed before a batch is run on any instance that receives the one-way sync, otherwise the promotions it applies are reverted. It already reads the created `match_id` from the approval response when present, so it needs no change once [OSDEV-3424](https://opensupplyhub.atlassian.net/browse/OSDEV-3424) ships.
...
* [OSDEV-3411](https://opensupplyhub.atlassian.net/browse/OSDEV-3411) - `SubmissionQualityProcessor` now logs, at `INFO`, the judged SLC submission fields (`name`, `address`, `country`, as received before cleaning) plus a short SHA-256 digest of the whole request body each time the LLM quality check runs, together with the contributor ID and the warning types raised (empty when nothing was flagged), and again when a resubmission bypasses the check via `ignore_warnings=true`. Comparing the two lines for a contributor in CloudWatch shows whether the warnings led to different data being submitted; the digest reveals whether any other field changed without recording its content. Free-text fields such as `notes` and source details are never logged. No response or persistence changes.
* [OSDEV-3437](https://opensupplyhub.atlassian.net/browse/OSDEV-3437) - `GET /api/facilities-download/` gains a `data_center_information` column, always present as the last column of a full (non-embed) download. For a facility classified as a data center (a `facility_type` ExtendedField resolving to "Data Center", the same rule `is_data_center` uses), the cell is a JSON object of every contributed data-center-specific field (`ExtendedField.DATA_CENTER_FIELDS`, OSDEV-2568 — capacity, PUE, power/water usage, named entities, building/area figures, operational status, etc.), keyed by field name; a field contributed more than once (e.g. from different list items) keeps every value as a list rather than the last write silently winning. Every other facility gets an empty cell. New stateless `DataCenterDownloadHelper` (`api/serializers/facility/data_center_download_helper.py`) reads the already-loaded `extended_fields` payload on each row, so no per-row database query is added to the paginated bulk download.
...
* [OSDEV-3459](https://opensupplyhub.atlassian.net/browse/OSDEV-3459) - `sync_databases` now runs `reassert_rba_promotions` at the end of each successful run on the RBA instance, so promotions the one-way sync just reverted are restored in the same job rather than waiting for someone to run the command by hand. The re-assert is what has to follow the overwrite; putting it in the sync command itself avoids a second schedule guessing how long a checkpointing sync takes, and a second Batch job that would need its own Django environment. `--dry-run` is passed through. Outside the RBA instance the hook is skipped, matching the command's own guard. A re-assert failure exits non-zero without being relabelled a sync failure; because it shares the `db_sync` job, a conflict can be retried with that job's existing retry policy (the restore is idempotent; a `created_from` collision is reproduced rather than cleared).
...
* [OSDEV-3224](https://opensupplyhub.atlassian.net/browse/OSDEV-3224) - Added CloudWatch error monitoring for AWS Lambda, resolving the Vanta SOC 2 test "Serverless function error rate monitored (AWS)", which was failing on nine ContriBot functions (`fetch_lists`, `process_list` and `notify` across Production, Staging and RBA). A new `alarm…LambdaErrors` in `deployment/terraform/alarms.tf` watches the `AWS/Lambda` `Errors` metric with **no `FunctionName` dimension**, so it covers every Lambda in the environment's region at once — the nine flagged functions plus `AlertBatchFailures`, `AlertStepFunctionsFailures`, `ContribotRetryFailedLists` and `NlbTargetsRegistrar` — and, more importantly, covers any function added later, so a new Lambda cannot silently re-fail the test the way a set of per-function alarms would. It pages when the 5-minute `Sum` exceeds the new `lambda_errors_alarm_threshold` variable (default **0**, i.e. any error), with `evaluation_periods = 1` and `treat_missing_data = notBreaching` because an idle environment publishes no datapoints at all. Notifications reuse the existing `aws_sns_topic.global` → AWS Chatbot → Slack path established by [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867), so there is no new alerting wiring and no tfvars change in any environment.
* [OSDEV-3402](https://opensupplyhub.atlassian.net/browse/OSDEV-3402) - **[SOC 2]** Dependency vulnerability remediation across the Django, Dedupe Hub, and React trees:
  * **Kafka client** — `aiokafka` 0.8.0 → 0.12.0 in `src/django/requirements.txt` and `src/dedupe-hub/api/requirements.txt`, with `kafka-python` dropped from both. aiokafka has vendored kafka-python's code since 0.9.0, so the separate pin no longer served a purpose — and it was actively blocking the bump, because aiokafka 0.8.0 imports `collect_hosts` from `kafka.conn`, which kafka-python relocated to `kafka.cluster` after 2.2.0 (the Django image failed at `collectstatic` with `ImportError`). Dedupe Hub's `app/main.py` now imports `TopicPartition` from `aiokafka` rather than `kafka`; `AIOKafkaProducer`/`AIOKafkaConsumer` call sites are unchanged, as neither service passes `api_version` or uses compression codecs.
  * **Django deps** — `src/django/requirements.txt`: `djangorestframework` 3.16.1 → 3.17.2, `djangorestframework-gis` 1.2.0 → 1.2.1.
  * **Dedupe Hub deps** — `src/dedupe-hub/api/requirements.txt`: `typing-extensions` 4.7.1 → 4.16.0, required by aiokafka 0.12.0 (`>=4.10.0`). Nothing else in the file constrains it — `fastapi[all]` 0.75.0, `dedupe`, `SQLAlchemy` 1.4.49 and `geoalchemy2` declare no bound, and pydantic 1.10.x asks only for `>=4.2.0` — and the `TypedDict` / `DefaultDict` imports in the matching DTOs and `gazetteer_matcher.py` are unaffected in 4.16.
  * **Frontend** — Floated `browserslist` to 4.28.8 via `src/react/yarn.lock` only, clearing GHSA-c83g-rgw3-j3cx and GHSA-73wf-gq98-2v4g (both `<=4.28.6`). The tree carried two copies; the stale 4.24.4 one was the vulnerable resolution, and both now dedupe onto 4.28.8, which `@babel/helper-compilation-targets` (`^4.24.0`) and `core-js-compat` (`^4.28.1`) already accept. No new `resolutions` pins, and no other package resolutions changed.
* [OSDEV-3223](https://opensupplyhub.atlassian.net/browse/OSDEV-3223) - **[SOC 2]** Upgraded every Python Lambda off the deprecated 3.8/3.10 runtimes (3.10 loses AWS support October 31, 2026; 3.8 already lost support in October 2024) to Python 3.13, the latest runtime with full `pandas`/`numpy` wheel support (3.14 has none yet). `runtime` bumped in Terraform for `alert_batch_failures`, `alert_sfn_failures` (`deployment/terraform/lambda.tf`, 3.8→3.13), the four ContriBot Lambdas — `contribot_fetch_lists`, `contribot_process_list`, `contribot_notify`, `contribot_retry_failed_lists` (`deployment/terraform/contribot_lambda.tf`, 3.10→3.13) — and `nlb_targets_registrar` (`database-private-link-provider/lambda-nlb-registrar.tf`, 3.10→3.13). `contribot_process_list`'s Makefile now cross-compiles its dependency zip against `PYTHON_VERSION = 3.13` so bundled `numpy`/`pandas`/`rapidfuzz` wheels match the runtime; the ContriBot CI test job and `pyrightconfig.json` follow suit. Also fixed four regex literals in `lib/contribot.py` that raised `SyntaxWarning` under Python 3.12+ (unescaped `\s`/`\.` in ordinary strings, now raw strings) — no behavioral change.
* [OSDEV-3369](https://opensupplyhub.atlassian.net/browse/OSDEV-3369) - Added a CloudWatch metric filter and alarm on the SLC submission quality check's fail-open log line (`Submission quality check failed` in the app log group), alerting through the existing SNS → Chatbot → Slack path. The check deliberately fails open — any Bedrock error is logged and the submission proceeds without warnings — so an outage is invisible to contributors and moderators; in production the check failed silently from Aug 28–31, 2026 because the production AWS account lacked the AWS Marketplace subscription for the Bedrock model (each new model must be enabled once per AWS account before the ECS task role, which holds only `bedrock:InvokeModel`, can invoke it). The alarm fires on a single failure within an hour (organic SLC volume is low, and systemic causes fail every call) and treats missing data as OK, since zero failures is the normal state. The custom metric is namespaced per environment because Production, Staging, and RBA share an AWS account. No application behavior changes and no tfvars changes are needed.
* [OSDEV-3370](https://opensupplyhub.atlassian.net/browse/OSDEV-3370) - New `claim_attachments_signer` IAM role (assumed by the app task role, `s3:GetObject` on the files bucket) signs the 60-second attachment download URLs; the app task definition gains the `CLAIM_ATTACHMENTS_SIGNING_ROLE_ARN` env var (unset falls back to default storage URLs, so local/MinIO dev is unaffected). The files-bucket policy denies any presigned request older than 15 minutes on the `claim_attachments/` prefix (`s3:signatureAge`), capping a leaked URL's usable life at the policy layer; other presigned consumers (facility-list downloads, partner icons in CloudFront-cached responses) are deliberately untouched.
* [OSDEV-3374](https://opensupplyhub.atlassian.net/browse/OSDEV-3374) - New `claim_attachments_reader_role_arns` Terraform variable (default empty): role ARNs listed there — the automated-claims Lambda execution role — get a cross-account `s3:GetObject` bucket-policy grant scoped to `claim_attachments/*`. With the default empty list the statement is not emitted at all.
* [OSDEV-3459](https://opensupplyhub.atlassian.net/browse/OSDEV-3459) - **Batch failure alerting extended to the sync queue.** The `AlertBatchFailures` rule matched only the default job queue, so a FAILED nightly database sync — including a promotion re-assert that fails at the end of that same job — raised no alert at all. Its event pattern now also covers the sync queue on the instance that runs it. Alerts continue to go to Rollbar via the existing Lambda; no new alerting path was added. The export_csv queue is still outside the pattern and is left for separate work.
* **Local/CI MinIO image moved off Docker Hub.** `docker-compose.yml` now pulls MinIO from `quay.io/minio/minio`, pinned to `RELEASE.2025-09-07T16-13-09Z`, instead of the unpinned `minio/minio:latest`. MinIO delisted its entire `minio` Docker Hub organisation on 11 September 2026 — both `minio/minio` and `minio/mc` return `object not found` from the Hub API, and an anonymous registry pull now 401s — which broke the **Code Quality** integration job for every pull request at the *Build & Run OS Hub* step (`pull access denied for minio/minio`), before any test ran. Runners that still held a cached layer kept passing for about an hour afterwards, so the breakage first looked intermittent rather than total. `quay.io/minio/minio` is MinIO's own mirror and still serves the same releases; the pinned image behaves identically and still ships `/usr/bin/mc`, which the service's `mc ready local` healthcheck depends on. Pinning to a release tag rather than re-chasing `latest` also stops a future upstream change from altering the local stack without warning. **Deployed environments are unaffected** — MinIO is only the local/CI stand-in for S3, appears nowhere in `deployment/`, and the `AWS_S3_ENDPOINT_URL` override that points at it is gated behind `DEBUG` in `oar/settings.py`; deployed environments use AWS S3 directly and run their containers from ECR.

### Bugfix
* [OSDEV-3462](https://opensupplyhub.atlassian.net/browse/OSDEV-3462) - A facility's **Facility Type** added by editing an already-approved claim's "Facility / Processing Types" now actually surfaces in the search filter. `api_facilityindex.facility_type` (fixed correctly by OSDEV-3430) was never the problem — the Logstash filter that builds the OpenSearch `location_type` field (`src/logstash/scripts/production_locations/location_type.rb`) read only the matched-taxonomy slot of each `matched_values` entry and silently dropped it when that slot was null, with no fallback to the raw submitted value. Claim-sourced facility types always skip taxonomy matching by design, so that slot is always null for them, meaning a claim-edited facility type was silently dropped from search every time, indefinitely — not a sync delay. The sibling `processing_type.rb` already read the raw-value slot and was unaffected. Fixed by falling back to the raw value when the matched slot is null, mirroring the fallback already present in the `index_facility_type()` SQL function.
* [OSDEV-3430](https://opensupplyhub.atlassian.net/browse/OSDEV-3430) - Approving or revoking a claim now refreshes the **Facility Type** and **Processing Type** search-filter columns. `index_facility_type`/`index_processing_type` count approved-claim extended fields, but the claim indexing trigger never recomputed the two columns, so a revoked claim's types kept appearing in the filters and an approved claim's types didn't appear until an unrelated event re-indexed the facility. Follow-up to OSDEV-3428.
* [OSDEV-3428](https://opensupplyhub.atlassian.net/browse/OSDEV-3428) - Locations approved via a moderation event (the v1 `production-locations` API and the SLC form) no longer come out invisible to the **Facility Type** and **Processing Type** search filters. Since 2.29, [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) made the `api_facilityindex.facility_type`/`processing_type` columns count only extended fields backed by an active `FacilityMatch` — but the approval path computed the columns before the match existed and no trigger recomputed them afterwards, so they stayed empty (the profile page, which reads extended fields directly, looked correct). Fixed twice over: the match, source, and list-item indexing procedures now recompute both columns whenever their tables change (which also fixes the columns going stale when a match or source is later deactivated), and the approval sequence stamps extended fields after the match is created. Locations approved between the 2.29 deploy and this release are re-indexed by the backfill command in the release instructions.
* [OSDEV-3352](https://opensupplyhub.atlassian.net/browse/OSDEV-3352) - `GET /api/facility-claims/{id}/` no longer raises `AttributeError` (HTTP 500) for decided claims whose `status_change_by` is null (legacy rows); the decider now serializes as null.
* [OSDEV-3367](https://opensupplyhub.atlassian.net/browse/OSDEV-3367) - **Destroy Environment** now empties the ECR repositories reliably, so Pre-prod teardown no longer fails at its final step with `RepositoryNotEmptyException` and no longer needs the repositories cleared by hand in the AWS console. Since [OSDEV-2646](https://opensupplyhub.atlassian.net/browse/OSDEV-2646) switched image publishing to `docker buildx bake --push`, buildx attaches a provenance attestation by default, so each commit tag is pushed as an OCI image index with two untagged children (the image manifest and the attestation). ECR refuses to delete a child while its index still references it, so `deployment/delete_ecr` — which passed every digest into one `batch-delete-image` call — could only ever strip the indexes and orphan the children, and its trailing `|| true` reported success anyway. The script now deletes in passes until `list-images` comes back empty, chunks digests to respect the 100-ID limit of `batch-delete-image` (the previous single call would have failed outright on any repository holding more than 100 images), and exits non-zero if a repository is still not empty, so the failure surfaces in `clean_ecr_repositories` in seconds instead of 23 minutes into `terraform destroy`.
* [OSDEV-3216](https://opensupplyhub.atlassian.net/browse/OSDEV-3216) - ContriBot no longer mishandles three contributor file shapes that were carried into the Lambda rebuild from the Airflow implementation. **Files above the 10,000-row limit are now rejected instead of trimmed.** Previously the frame was cut to 10,000 rows and processing continued, but the report's row count and error ratio described the *truncated* frame, so it read as a complete review of the file; approving on that basis ingested every row, including those nothing had examined. `process_list` now fails such a list, so it is marked `FAILED`, routed to `notify`, and the contributor can be asked to split the file. (The limit itself stays: the duplicate scan compares every row pair, so cost grows with the square of the row count.) A file with no `address` column raised `AttributeError: no attribute 'address'` in `check_name_address_duplicates` and `check_for_country_name_in_address`, aborting the run even though the missing column had already been recorded as `T0003`; both now check the column is present. The unrecognised-column finding (`T0008`) passed `column=` where the signature takes `column_name=`, leaving the finding's column name blank — both are now passed, since the record needs one and the message template renders the other. Regression tests cover all three.
...
* [OSDEV-3224](https://opensupplyhub.atlassian.net/browse/OSDEV-3224) - Fixed the two alerting Lambdas, `func…AlertBatchFailures` and `func…AlertStepFunctionsFailures`, failing every invocation with `Runtime.ImportModuleError: No module named 'alert_sfn_failures'`. Three faults compounded. The committed archives were built by hand from the wrong directory in 2023, so each one held `Makefile`, `requirements.txt` and `src/<name>.py` — putting the handler module one level below the archive root, where the configured handler `<name>.handler` cannot find it — and carried no bundled dependencies at all, so `import rollbar` would have failed next. Each function's `Makefile` has always produced the correct layout (`cd src && zip -qXr ../<name>.zip .`, after pip-installing requirements into `src/`), and `deployment/infra plan` has run it on every deploy since, but **`source_code_hash` was commented out** on both `aws_lambda_function` resources: with nothing describing the archive's contents, a rebuilt zip was never a diff and `apply` never re-uploaded it, so the 2023 bytes stayed deployed for three years. `source_code_hash` is now active on both. The archives are also gitignored — a zip is a build artifact, and committing one is what let these rot — and `deployment/infra` now treats them like the ContriBot archives: `CONTRIBOT_LAMBDAS` becomes `LAMBDA_ARCHIVES` with the two alert functions added, which removes the special-cased `make` calls and gives them the same build-at-plan → S3 → byte-identical-at-apply handoff, plus the build-before-destroy that `filebase64sha256()` now requires of them. The two Lambda@Edge archives are deliberately excluded: they have no `Makefile` and stay committed. Both functions also move from the `python3.8` runtime (deprecated Oct 2024; AWS blocks function updates from Mar 3, 2027) to `python3.13`, and `rollbar` is bumped `0.16.3` → `1.4.0` to match — 0.16.3 declares support only through Python 3.8, while 1.4.0 declares `Requires-Python >=3.10` and classifiers through 3.14. No code change was needed: the three APIs these functions use (`rollbar.init`, the `@rollbar.lambda_function` decorator, `rollbar.report_message`) are unchanged in 1.4.0. `func…AlertStepFunctionsFailures` is wired into the state machine at `sfn.tf`, so it only runs when a state machine has already failed — rare enough that a broken alerter went unnoticed for three years, and precisely the class of silent failure the new `alarm…LambdaErrors` above is there to catch.

### What's new
* [OSDEV-2278](https://opensupplyhub.atlassian.net/browse/OSDEV-2278) - Claimants can now view and edit their own pending facility claims directly on OS Hub: My Account > My Facilities lists PENDING claims alongside approved ones (status chip per row), and opening a pending claim shows the full claim form for updating any detail plus a documents section for uploading new evidence or removing a document — replacing the email-based document exchange that raised the PII/SOC 2 concern. The Claims Team is notified on every claimant save.
...
* [OSDEV-3374](https://opensupplyhub.atlassian.net/browse/OSDEV-3374) - Optional per environment: set `claim_attachments_reader_role_arns` in tfvars to the automated-claims Lambda execution role ARN to enable direct S3 attachment reads (the pipeline falls back to the API download endpoint until this is set).
* The private instance needs this release's Batch image deployed before `sync_databases` will re-assert promotions (OSDEV-3459). **Deploy to AWS** with `deploy-mode` set to `terraform-plan-and-apply` is also required so the widened `AlertBatchFailures` event pattern covers the sync queue; other environments apply only that alert change.
...
* `0226_add_slc_submission_quality_check_switch.py` - Adds the `slc_submission_quality_check` waffle switch (created active, so behavior is unchanged on deploy) that controls the LLM-backed SLC submission quality check. See OSDEV-3118.
* `0227_facilitylistitem_ai_usage_notes_and_more.py` - Adds the seven per-row provenance columns to `api_facilitylistitem` (`source_name`, `source_link`, `information_source_type`, `date_of_source`, `notes`, `data_collection_methodology`, `ai_usage_notes`), all nullable and blank-able so every existing row and every non-data-center contribution is unaffected. Also updates the `field_name` choices on `ExtendedField` and `HistoricalExtendedField` to cover the 57 data-center field names. The choices update is an `AlterField` on validation metadata only — no column type change, no backfill, and no reindex. See OSDEV-3070 / OSDEV-3071, OSDEV-3066 and OSDEV-3233.
* `0228_create_polygon.py` - Adds the `Polygon` table: a reusable, named geographic boundary (an auto-generated unique `uuid` for joining, unique variable-style `name` — letters, digits, and underscores only — an optional human-friendly `display_name`, `description`, and a `geom` `MultiPolygonField` in EPSG:4326) that is not tied to country borders. Polygons may be any shape, including multiple disjoint parts and polygons with holes. `Polygon.get_production_locations()` returns the OS Hub production locations within a given boundary via the existing `location__within` PostGIS lookup, optionally narrowed by an allowlist of core-field filters (e.g. `{'country': ['IN'], 'sector': ['Apparel']}`) applied in the same query. Boundaries are added through the Django admin by pasting or uploading GeoJSON (a bare `Polygon`/`MultiPolygon`, a `Feature`, or a `FeatureCollection`, which are parsed, merged, and validated before saving); GeoJSON that declares a non-WGS-84 coordinate system, or whose coordinates fall outside valid longitude/latitude ranges, is rejected with a clear form error. This is foundational infrastructure for future boundary-driven features (e.g. per-region partner fields) rather than a user-facing change on its own; related context in [OSDEV-3125](https://opensupplyhub.atlassian.net/browse/OSDEV-3125).
* `0229_add_partner_field_polygon_link.py` - Adds an optional `polygon` link (a ForeignKey) from `PartnerField` to `Polygon`, so a polygon-driven partner field points at its coverage boundary through the database. The link is rename-proof, and deleting a polygon a field points at is blocked.
* `0230_setup_india_labour_line_partner_field.py` - Creates the `india_labour_line_helpline` partner field, or adopts and normalizes the manually-created production row (it is not deleted, so its contributor assignment survives): protected system field mirroring `mit_living_wage`, helpline number in `display_text` (which stays admin-editable on protected fields), coverage polygon linked automatically when a polygon named `india_labour_line_helpline_areas` exists.
* `0231_index_facility_processing_raw_values.py` - Rewrites the facility and processing type index functions and adds supporting indexes so unmatched contributor-submitted raw values remain searchable. The indexes retain only profile-visible values from an approved claim or an active match with an active source, and exclude placeholder and legacy junk values. See OSDEV-3189.
* `0232_create_facility_processing_value_index.py` - Adds trigger-maintained case-insensitive facility and processing type value aggregates, including distinct-location counts and submitted-casing variants used by processing type suggestions. Runs non-atomically in two steps so the `SHARE ROW EXCLUSIVE` lock that `CREATE TRIGGER` takes on `api_facilityindex` is released before the initial count rebuild, which would otherwise block all indexing of that table for the duration of the rebuild. See OSDEV-3189.
* `0233_index_facility_processing_search.py` - Adds GIN indexes on the `facility_type` and `processing_type` arrays of `api_facilityindex`, on their lower-cased forms, and a trigram index on both columns combined, plus the immutable `immutable_unaccent`, `lower_varchar_array`, and `facility_processing_search_text` helper functions the expression indexes are built on. Also replaces the raw-value trigram index on `api_facility_processing_value` with a `btree_gin` index over `(kind, immutable_unaccent(lower(value)))`, which is the expression the typeahead actually searches. Every index is created `CONCURRENTLY` on a non-atomic migration, so contributions are not blocked while they build. See OSDEV-3189.

#### Schema changes
* [OSDEV-3142](https://opensupplyhub.atlassian.net/browse/OSDEV-3142) - Added the `is_anonymized` column to `api_source`.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `api_facility_processing_value` and `api_facility_processing_value_variant` aggregate tables with triggers that keep suggestion values and distinct-location counts synchronized with `api_facilityindex`.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added five GIN indexes on `api_facilityindex` covering the `facility_type` and `processing_type` arrays, their lower-cased forms, and their combined trigram text. Together they add about 50 MB to a 12 GB table, most of it the trigram index.
* [OSDEV-3075](https://opensupplyhub.atlassian.net/browse/OSDEV-3075) - Added seven nullable provenance columns to `FacilityListItem`: `source_name` (500 chars), `source_link` (URL, 2000 chars), `information_source_type` (200 chars), `date_of_source` (10 chars), and the free-text `notes`, `data_collection_methodology` and `ai_usage_notes`. They sit on the row rather than on `Source` because one uploaded list can aggregate rows drawn from many different external sources, so a per-contribution field could not describe them. `information_source_type` is the data-center schema's `source_type`, renamed to avoid colliding with the unrelated `Source.source_type` (which records the ingestion channel, `LIST` or `SINGLE`).
* [OSDEV-3066](https://opensupplyhub.atlassian.net/browse/OSDEV-3066), [OSDEV-3233](https://opensupplyhub.atlassian.net/browse/OSDEV-3233) - Registered 57 data-center attribute names in `ExtendedField.FIELD_CHOICES` via a new `DATA_CENTER_FIELDS` tuple. No new table and no new column: data-center attributes are stored as ordinary community-contributed `ExtendedField` rows, so one contributor can supply power figures while another supplies building dimensions for the same location, and each value keeps its own contributor attribution and history.

### Code/API changes
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Added `GET /api/processing-type-suggestions/` and extended facility list, download, and tile filtering with ranked Facility Type / Processing Type search. Suggestions include taxonomy labels and contributor-submitted values, rank taxonomy children for selected facility types, combine case-only variants under canonical labels, and apply case-insensitive whole-value matching for selected suggestions while preserving punctuation and accent distinctions. The React extended-search sidebar now provides autocomplete results, hierarchy context, location counts, multi-value chips, keyboard navigation, and URL-persisted exact selections.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Made every Facility Type / Processing Type filter on facility search index-assisted. `api_facilityindex` had no index on either array column, so all of these filters — including the pre-existing taxonomy ones — scanned the whole 12 GB table. Taxonomy values keep using array overlap, now served by a plain GIN index; the case-insensitive filter behind a selected suggestion overlaps the lower-cased array (`lower_varchar_array(processing_type) && …`) instead of lower-casing each element inside an `unnest`, which could not use an index; and free-text values gather candidates from the trigram index on `facility_processing_search_text(facility_type, processing_type)` before the word-prefix regex rechecks them, a substring match being a superset of a word-prefix match. The relevance score that orders free-text results now evaluates both its whole-word and word-prefix tiers in a single pass over the two arrays rather than as four separate `EXISTS`. Measured over 2.5M locations: a selected suggestion 2679 ms → 2 ms, a free-text term 3913 ms → 250 ms, a taxonomy value → under 1 ms. Results are unchanged; the LIKE pattern is normalized by the same functions the index expression uses, so accented and unaccented spellings still match the same locations.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - `index_facility_type()` and `index_processing_type()` read each location's ExtendedFields in a single pass instead of one per value branch, and pair a matched value with its raw counterpart by subscripting the raw array at the matched value's position rather than cross-joining both expansions and discarding the mismatched pairs. Both functions run on every ExtendedField insert, update and delete, so this is a write-path saving: the location with the most ExtendedFields drops from 20.2 ms to 8.4 ms and a 2,400-location batch from 1.08 s to 0.81 s. The rewritten functions intentionally change output: they include unmatched contributor-submitted raw values, retain only profile-visible values from an approved claim or an active match with an active source, and exclude placeholder and legacy junk values.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - The processing type typeahead's candidate query now matches on `immutable_unaccent(lower(value))` rather than `unaccent(value) ILIKE …`, so it can use an index; the previous predicate read every row of `api_facility_processing_value` and left the trigram index on the raw `value` column unused. A typical query drops from 3.5 ms to 0.7 ms over 12,472 values. Typed-query suggestions and ranking are unchanged; an empty query now splits result slots evenly between taxonomy and contributor values, gives taxonomy the odd slot, and backfills unused slots from either group.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - `recompute_facility_processing_values()` now unnests `api_facilityindex` once into a temporary table and derives both the submitted-variant and the lower-case identity counts from it, instead of scanning the 12 GB table twice. The counts it produces are identical; the identity counts still cannot be summed from the variant counts, because a location carrying two casings of the same value has to count once.
* Added the India Labour Line helpline as a system partner field (`india_labour_line_helpline`), mirroring the MIT Living Wage pattern but keyed on `Polygon` boundaries instead of US counties: production locations in India whose coordinates fall inside the coverage polygon their partner field links to, and that work in a covered sector (Apparel, Apparel Accessories, Footwear, Home Textiles, Leather, Textiles), get the helpline phone number (read at request time from the partner field's `display_text`, which stays editable in the Django admin even on protected system fields — no deploy needed to change the number). The partner's contributor-profile spotlight and the Spotlight Data Partners search filter both resolve via PostGIS polygon containment. The feature activates through configuration alone (the boundary polygon and the partner field assignment); missing configuration (an unlinked polygon or a blank number) disables the feature with a logged warning rather than erroring or over-matching. Requires no front-end changes.
* [OSDEV-3118](https://opensupplyhub.atlassian.net/browse/OSDEV-3118) - Added AI-powered data-quality warnings and a blocking separator check to the SLC (Single Location Contribution) create flow, building on the duplicate-check/override pattern from OSDEV-2980. A new `SubmissionQualityProcessor` in the location contribution pipeline (after `DuplicateSubmissionProcessor`, before `GeocodingProcessor`) makes a single LLM call (Claude Haiku by default) via AWS Bedrock — the first LLM integration in the Django backend, using the vendor-independent `pydantic-ai` library over Bedrock's Converse API with a pydantic output schema for validated structured output, so the model is an env-var swap and the provider is not hardcoded to one vendor's SDK — returning verdicts for four checks in one round trip: implausible facility name, implausible or under-specified address, address/country mismatch, and multiple locations bundled into one submission. If any verdict flags, `POST /api/v1/production-locations/` returns `409` with a new generic `warnings: [{type, title, message}]` array (kept separate from the existing `errors`/`duplicate_of` contract) instead of creating a moderation event; passing `?ignore_warnings=true` (only the literal values `true`/`false` are accepted; anything else returns `400`) skips the LLM call entirely and proceeds. Warnings are computed per request and returned in the response only — never stored on `ModerationEvent`. The check fails open: on Bedrock timeout (5 seconds) or any error, the failure is logged and the chain proceeds as if the check had not fired, so a slow or broken LLM never blocks a legitimate submission. Scoped to SLC-sourced `CREATE` submissions only — API contributions and `PATCH` updates are never evaluated. On the SLC form: each `productType`, `locationType`, and `processingType` value now rejects an embedded vertical bar (`|` — the backend's own multi-value join character, so a value containing it would be re-split downstream) as a hard inline validation error that disables Submit with no override (a new `no-embedded-separators` test in the SLC validation registry, run before the Latin-characters test so a `|` gets the specific "enter as separate values" message; commas, slashes, ampersands, and the word "and" are allowed, since they appear in legitimate single values); all warnings within a tier are shown together in a single dialog instead of one at a time (client-detected PO Box/short-address warnings before any request is sent, server-returned quality warnings in a second dialog), with `ContributionWarningDialog` generalized to accept a `warnings` array; and overrides accumulate across dialogs within one submission attempt, so a submission that trips both the duplicate check and the quality check resends every override already granted (`duplicate_override` and/or `ignore_warnings` together) — previously such a submission would bounce between the two dialogs indefinitely. Each successful quality-check call also logs its Bedrock token usage (`Submission quality check tokens: input=… output=…`) so real per-call consumption is visible in CloudWatch Logs. The entire check sits behind a new `slc_submission_quality_check` waffle switch — created active by migration `0226`, so deploying changes nothing — letting it be turned off in the Django admin without a deploy; if the switch row is ever missing, waffle's default (`False`) disables the check rather than blocking submissions.
* [OSDEV-3142](https://opensupplyhub.atlassian.net/browse/OSDEV-3142) - Sources created for approved SLC (Single Location Contribution) moderation events are now anonymized at creation via a new `Source.is_anonymized` flag (also editable per source in the Django admin). Unlike `is_public` — whose semantics are unchanged and which hides an anonymized source's names and addresses from the index entirely — `is_anonymized` keeps every piece of contributed data visible (names, addresses, sectors, extended fields, alternate map locations) and hides only the contributor attribution: the contributor appears in the facility's contributor list aggregated anonymously by contributor type (e.g. "A Service Provider"), field-level attribution falls back to the contributor-type phrase with no contributor id, the facility is not returned when filtering search by that contributor (`index_contributors_id`), CSV/XLSX downloads show the anonymous form, and the `is_from_claim` badge is suppressed for anonymized contributions from an approved claimant (naming the claim would undo the anonymization by inference). Applies when a moderator approves an SLC event via `POST /api/v1/moderation-events/{moderation_id}/production-locations/` (new location) or `PATCH /api/v1/moderation-events/{moderation_id}/production-locations/{os_id}` (contribution to an existing location), only to moderation events with source `SLC`; API-sourced events approved through the same endpoints are unaffected. SLC contributions from an approved claimant of the target location are exempt (checked at approval time): the claimant is already publicly named on the location, so their contributions stay attributed and keep the claim promotion. Controlled by the new `anonymize_slc_sources` waffle switch, which defaults to **on**; turning it off restores attributed SLC sources. Existing sources are not modified.
...
* [OSDEV-3170](https://opensupplyhub.atlassian.net/browse/OSDEV-3170) - Extended the "Claimed" data attribution from OSDEV-3093 to name, address, and sector. Those three are not served by the extended-field serializer — contributed names and addresses come from `FacilityIndex.facility_names`/`facility_addresses` and sectors from `item_sectors` — so an approved claimant's own SLC or list-upload submissions were still labeled "Crowdsourced" on the production location page and in `GET /api/facilities/{id}/`, directly beside the claimant's own organization as the credited source. `create_name_field_from_facility_name`, `create_address_field_from_facility_address`, and `format_sectors` now mark an entry as claim data when its contributor matches the facility's approved claim contributor (`approved_claim.contributor_id`), via the shared `is_contribution_from_claimant` helper. Masked (anonymized) contributions keep the claim attribution while the contributor's identity stays hidden — the label states where a value came from, and the existing contributor name/id masking hides who supplied it. This also changes the OSDEV-3093 behavior released in 2.28, which excluded masked contributions and so labelled one claimant's data differently depending on the channel it arrived through — claim-form fields were always marked as claim data regardless of masking, so the exclusion was never consistent. **No displayed value changes:** the name and address rows show the canonical `properties.name`/`properties.address`, which this change does not touch (marking a claimant entry does raise it within the name/address lists, which is what makes the row credit the claimant instead of another contributor of the same value); the sector list order is left exactly as it was, so entries are marked in place and the displayed sector never moves. Promoting a claimant's value above other contributors' — including a claimant sector behind a more recently contributed one — remains out of scope and is tracked under OSDEV-392. Unclaimed facilities are unchanged; no migration or reindex is required.
* [OSDEV-2587](https://opensupplyhub.atlassian.net/browse/OSDEV-2587) - Added `Data Center` to the facility-type taxonomy (`api/facility_type_processing_type.py`) with the aliases `data center`, `data centre`, `datacenter`, `datacentre` and `dc`. A location is classified as a data center purely by its `facility_type` extended field resolving to "Data Center" — there is no discriminator column — so the value rides the existing `facility_type` pipeline and surfaces as `location_type` in the v1 API and the OpenSearch index, making data centers filterable through the existing facility-type/location-type search filter with no new filter code. The lookup is resolved before the sector gate in `get_facility_and_processing_type`, because that gate returns `SKIPPED_MATCHING` for anything outside the Apparel taxonomy and would otherwise have dropped every data-center value. Existing production facilities are unaffected: `DATA_CENTER` maps to an empty processing-type set, so no existing processing type resolves to it.
* [OSDEV-3067](https://opensupplyhub.atlassian.net/browse/OSDEV-3067) - Added `is_data_center()` in the new `api/helpers/data_center.py` as the single definition of "is this location a data center", parsing the `{raw_values, matched_values}` shape of the `facility_type` extended field. `facility_type` is multi-valued, so a location counts as a data center when *any* contributed value resolves to "Data Center"; a location asserted as both a data center and a production type is treated as a data center. All callers use the helper rather than inlining the check.
* [OSDEV-3066](https://opensupplyhub.atlassian.net/browse/OSDEV-3066) - Registered the data-center attribute fields as community-contributed `ExtendedField`s, so they flow through the existing contributions pipeline for list upload, the legacy API and v1/SLC with no per-field ingestion code. The names were added to `RAW_DATA_FIELDS` and handled by a data-center branch in `create_extendedfield` that stores a generic `{raw_value}` shape, normalising the numeric fields listed in `DATA_CENTER_NUMERICAL_FIELDS` so a whole number stores as an integer and a fractional one keeps its precision. Fields that already have a home — `country`, `address`, `lat`, `lng`, `name_site`, `name_parent_company` and `number_of_workers` — reuse the existing column or extended field rather than gaining a duplicate.
* [OSDEV-3233](https://opensupplyhub.atlassian.net/browse/OSDEV-3233) - Aligned the registered extended fields with the current data-center schema. Added `wue_units` and `onsite_power_generation` to utility usage, and `non_data_area(_units)`, `number_of_floors`, `footprint(_units)`, `building_area(_units)`, `land_area(_units)` and `number_of_buildings` to building information. Removed `overall_area` and `overall_area_units`, which the schema replaced with `building_area` / `land_area` / `footprint`; `ExtendedField` rows already stored under those names are intentionally left in place (removing a name from `FIELD_CHOICES` does not delete rows) and simply stop rendering. Also added two grouping fields, `is_group` (boolean) and `data_center_group_id` (string). These are deliberately a **staging mechanism**, not the grouping model: they let collectors record which building or campus a data center belongs to ahead of [OSDEV-3083](https://opensupplyhub.atlassian.net/browse/OSDEV-3083)–[OSDEV-3087](https://opensupplyhub.atlassian.net/browse/OSDEV-3087), so a later script can create the groups and relationships from the recorded ids. Because they are extended fields they are contributor-asserted and multi-valued — two contributors can assert different group ids for the same location and nothing enforces one-group-per-location — so, like `is_data_center`, they must not be used to gate behaviour such as merge/link guards or filters.
* [OSDEV-3075](https://opensupplyhub.atlassian.net/browse/OSDEV-3075) - Per-row provenance is now captured at all three `FacilityListItem` creation sites — list upload (`processing_facility_list.py`), the legacy API (`processing_facility_api.py`) and SLC moderation-event approval (`event_approval_template`) — through a single shared `extract_provenance()` helper, so the three paths cannot drift. Values are validated centrally before persistence: non-strings are rejected, values are trimmed and dropped when they exceed their column, and `source_link` must pass Django's `URLValidator` restricted to `http`/`https` (the value is rendered as a clickable link, so other schemes are never stored). `date_of_source` accepts ISO 8601 reduced precision — `YYYY`, `YYYY-MM` or `YYYY-MM-DD` — because external sources frequently state only a year or a month. An invalid value is logged and omitted rather than failing the row, since provenance is supplementary metadata and rejecting the row would discard the location data with it. The schema's `added_on_date` maps to the existing `created_at`, and `added_by` / `added_by_type` to the source's contributor.
* [OSDEV-3068](https://opensupplyhub.atlassian.net/browse/OSDEV-3068) - Both contribution APIs now accept data-center attributes and provenance. On the legacy `POST /api/facilities/`, the fields ride the existing raw-data path and need no per-field wiring. On v1 `POST /api/v1/production-locations/` and `PATCH /api/v1/production-locations/{os_id}`, the seven provenance fields were added to the production-location schema serializer as deliberately lenient free text — recorded as provided, with `max_length` limits that only mirror the column sizes so an accepted value can always be stored. The one exception is `date_of_source`, which is validated and normalised to ISO reduced precision and returns a `422` with a format message otherwise. Provenance travels with the moderation event and lands on the `FacilityListItem` when the event is approved.
* [OSDEV-3073](https://opensupplyhub.atlassian.net/browse/OSDEV-3073) - `GET /api/facilities/{os_id}` now returns a `provenance` object on each extended-field entry, resolved from the `FacilityListItem` the value was contributed by. It is `null` when the originating row carries no provenance or the field has no list item (for example claim-born fields). Implemented in `FacilityIndexSerializer` as a single bulk lookup keyed on the `facility_list_item_id` already present in the indexed entries, so there is no `index_extended_fields()` SQL change and no reindex. It is scoped to the details endpoint by an `include_extended_field_provenance` flag that defaults to `False` on the base serializer and is switched on only by `FacilityIndexDetailsSerializer`, so list and search endpoints take no extra queries. Note that `name`, `address` and `sector` are synthesised from the index's `facility_names` / `facility_addresses` arrays, which carry no list-item id, so those three fields report no provenance. The same endpoint also returns `is_data_center`.

### Architecture/Environment changes
* [OSDEV-2413](https://opensupplyhub.atlassian.net/browse/OSDEV-2413) - **[Release] Init** and **[Release] Deploy** now authenticate with `GITHUB_TOKEN` instead of a user-owned `PAT`. Init only creates `releases/v.*` (it does not start **Deploy to AWS**; Pre-prod stays a later manual run, as in OSDEV-1925). Deploy creates the env tag and `workflow_dispatch`es **Deploy to AWS**, which `GITHUB_TOKEN` is allowed to do. Init needs `contents: write`; Deploy needs `contents: write` and `actions: write`. Removed the **Deploy to AWS** `check-base` job and `scripts/check_release_branch.sh`: that skip existed to ignore the empty `releases/*` push from Init when it used a PAT; `GITHUB_TOKEN` no longer fires that push, and the branch is never created by hand. Those two workflows now use `actions/checkout@v7` and Init uses `peterjgrainger/action-create-branch@v4.0.0` (Node 24), taking the Node 20 runner deprecation warnings from three down to one: **[Release] Deploy** still uses `mathieudutour/github-tag-action@v6.2` (no newer release exists).
* [OSDEV-3118](https://opensupplyhub.atlassian.net/browse/OSDEV-3118) - Provisioned AWS Bedrock access for the SLC submission-quality check: added `pydantic-ai-slim[bedrock]` to the Django requirements (bumping `boto3` from 1.35.37 to 1.42.63, its floor) and a new `bedrock_invoke_submission_quality_model` IAM policy in `deployment/terraform/iam.tf`, attached to the existing Django ECS task role (`app_task_role`). The policy's model grants are driven by new terraform variables (`bedrock_submission_quality_region`, `bedrock_submission_quality_inference_profiles`, `bedrock_submission_quality_foundation_models`) defaulting to the `eu.anthropic.claude-haiku-4-5-20251001-v1:0` cross-region inference profile plus its region-wildcarded foundation-model ARN (required because the profile routes across regions; Bedrock rejects the bare foundation-model ID) — so swapping or A/B-ing models is a tfvars + env-var change, not a policy rewrite. The system-level instructions for the LLM call (task framing, strictness, tone) are likewise overridable per environment via a new optional `SUBMISSION_QUALITY_INSTRUCTIONS` env var (empty/unset uses the default baked into `SubmissionQualityService`); the meaning of each individual check stays defined by the output schema in code, since it is structurally coupled to the verdict fields and warning mapping. Auth uses the task's IAM role — no API key or secret to manage. For local development, a new optional `BEDROCK_AWS_PROFILE` env var (`.env.sample`) points the Bedrock call at a developer's real named AWS profile, because the Django container hardcodes `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` to MinIO's dummy values, which would otherwise shadow real credentials; leave unset in production.
* [OSDEV-3118](https://opensupplyhub.atlassian.net/browse/OSDEV-3118) - Added cost/volume monitoring for the SLC submission-quality check's Bedrock calls, deliberately instead of an in-app call cap: per-user volume is already bounded by the endpoint's `DataUploadThrottle` (30/minute), and the accepted-risk rationale is documented in `doc/ops/monitoring.md`. A new CloudWatch alarm `alarm…BedrockInvocations` (`AWS/Bedrock` `Invocations`, hourly `Sum` > `bedrock_invocations_alarm_hourly_threshold`, default 100 — orders of magnitude above organic volume; no `ModelId` dimension so future models/callers are covered) pages via the existing SNS → Chatbot → Slack path, with `treat_missing_data = notBreaching` because zero calls in an hour is the normal state. An account-level monthly AWS Budget on Bedrock spend (`budget…Bedrock`, `bedrock_cost_budget_monthly_limit_usd`, default $25) alerts at 80% actual and 100% forecasted spend through the same topic; budgets are account-wide, so only the account-owner environments create one (`manage_bedrock_cost_budget = true` in the Test and Production tfvars, mirroring the Chatbot channel-config ownership pattern). `aws_sns_topic.global` gains an explicit topic policy that restates the default owner-account access and adds a `budgets.amazonaws.com` publish grant, which AWS Budgets requires. Also restored the closing brace on `bedrock_submission_quality_foundation_models` in `deployment/terraform/variables.tf`, lost in a merge from `main`, which would have failed Terraform parsing.
* [OSDEV-3234](https://opensupplyhub.atlassian.net/browse/OSDEV-3234) - The ContriBot `process_list` Lambda now accepts contributor `.csv` uploads as well as `.xlsx`. After S3 download, CSV is converted to a temporary workbook (UTF-8 with optional BOM; delimiter sniffed) and both CSV and native Excel inputs are staged as `{list_id}.xlsx` so the Google Drive report is always `{list_id}.~PROCESSED.xlsx` (moderation tooling reads the facility-list id from the filename). Empty, header-only, or malformed CSVs fail the task so Step Functions can route to `notify`.
* [OSDEV-2640](https://opensupplyhub.atlassian.net/browse/OSDEV-2640) - **Code Quality** Flake8 and ESLint/Prettier jobs now run on the GitHub-hosted runner (`setup-python` 3.11 / `setup-node` 22) instead of `docker compose build` plus a container exec. Flake8 still uses the versions pinned in `src/django/requirements.txt` (`flake8`, `mccabe`, `pycodestyle`, `pyflakes`) and the same `src/django` excludes; ESLint/Prettier still lint `src/react/src/**/*.js(x)` after a Yarn install from `yarn.lock`. These two jobs no longer need the Quality Environment secret or a full app image build.
* [OSDEV-3187](https://opensupplyhub.atlassian.net/browse/OSDEV-3187) - Added a ContriBot retry path for facility lists that fail validation. `process_list` now records DynamoDB `FAILED` (and `finished_at`) on error, then re-raises so Step Functions Catch still routes to `notify`. A new `retry_failed_lists` Lambda scans for `FAILED` rows under `CONTRIBOT_MAX_ATTEMPTS` (default 3), resets them to `PENDING`, and starts the existing state machine with `{"lists": [...]}`. A Choice state at the top of the workflow skips `fetch_lists` when that payload is present, so the API cursor is never moved on retry. A second EventBridge rule invokes the retry Lambda on the same cadence as fetch. The retry Lambda uses its own IAM role (`states:StartExecution` plus DynamoDB) to avoid a Terraform cycle with the state machine. PR CI now runs ContriBot pytest under `src/contribot/*/tests`.
* [Follow-up][OSDEV-3187](https://opensupplyhub.atlassian.net/browse/OSDEV-3187) - `deployment/infra` now builds, uploads, and downloads the `contribot_retry_failed_lists` Lambda zip from the same `CONTRIBOT_LAMBDAS` list as the other ContriBot functions. Terraform plan was failing with `filebase64sha256(): no such file or directory` because that zip is gitignored and was never produced at plan time.
* [OSDEV-2646](https://opensupplyhub.atlassian.net/browse/OSDEV-2646) - **Deploy to AWS** now builds and pushes ECR service images in parallel via `docker buildx bake` (one job, shared ECR login) instead of sequential `docker/build-push-action` steps. Bake runs in two waves so Batch (which `FROM`s the Django image at the same commit tag) builds only after Django is in ECR; other images stay concurrent in wave 1. Immutable ECR tags are still checked up front and already-present SHA tags are omitted from the bake target list; production-only and test-only Dockerfiles remain env-conditional. Removed the redundant `update_services` job (`force-new-deployment`): Terraform apply already registers a new task definition revision and ECS rolls out automatically when the service's task definition changes (no deployment circuit breaker).
* [OSDEV-2645](https://opensupplyhub.atlassian.net/browse/OSDEV-2645) - **Deploy to AWS** now runs `build_and_push_react_app` and `build_and_push_docker_image` in parallel after Terraform apply (each depends only on `apply`; `create_kafka_topic` waits on both). Frontend and backend may finish slightly apart during that window.
* [OSDEV-2929](https://opensupplyhub.atlassian.net/browse/OSDEV-2929) - First stage of transitioning deployment secrets from the private `ci-deployment` tfvars repo to AWS Secrets Manager (CLI-owned `oshub/<env>/<purpose>` secrets). Public `deployment/environments/terraform-*.tfvars` hold SM secret names plus non-sensitive deployment config; **Deploy to AWS** and **Destroy Environment** no longer check out or merge `ci-deployment`. Terraform, CI/CD, ECS, and Batch read secret values from SM instead of private tfvars.
* [OSDEV-3185](https://opensupplyhub.atlassian.net/browse/OSDEV-3185) - Implemented the ContriBot `process_list` Lambda: downloads each enqueued list’s `.xlsx` from S3 using `file_name` as the object key, runs bundled `ContriBot` validation (`lib/0000.error_codes.xlsx`), uploads the `.~PROCESSED.` report to Google Drive, and returns `{list_id, report_url, num_lines, num_errors, error_ratio}` for `notify`. The process_list zip bundles `lib/` plus `requirements.txt`; Lambda memory raised to 1024 MB.
* [OSDEV-2928](https://opensupplyhub.atlassian.net/browse/OSDEV-2928) - Finished the remaining ContriBot Lambdas. `fetch_lists` resumes from a DynamoDB cursor, enqueues new facility lists, and feeds Step Functions. `notify` posts Slack (separate success and failure webhooks) and, after a successful `process_list`, creates a Monday approval-queue item. Catch/failure skips Monday; a Slack outage is logged, a Monday failure is re-raised. DynamoDB is marked `PROCESSED` or `FAILED`.
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) / [OSDEV-3236](https://opensupplyhub.atlassian.net/browse/OSDEV-3236) - Enabled `pgaudit` database activity auditing on the RDS for PostgreSQL instances, resolving the Vanta SOC 2 test `aws-rds-pgaudit-enabled`. `aws_db_parameter_group.default` in `deployment/terraform/database.tf` now sets `shared_preload_libraries` to `pg_stat_statements,pgaudit` and `pgaudit.log` to `ddl,role`, both `pending-reboot` because `shared_preload_libraries` is a static parameter. The values come from new `rds_shared_preload_libraries` and `rds_pgaudit_log` variables, validated at plan time so an override cannot drop either library or set an unsupported audit class. `ddl,role` audits schema changes and privilege/role grants; `read` and `write` are excluded on log-volume grounds. `pgaudit` was added to `install_db_exts`, which now exits non-zero instead of swallowing failures. All six environments run a single-instance RDS PostgreSQL 16 database with no Aurora cluster, so the Aurora acceptance criteria do not apply. **Rollout is two-phase and the order is load-bearing**: `pgaudit.log` ships as `none` (OSDEV-2997), because `CREATE EXTENSION pgaudit` installs the event triggers that supply object type and name for DDL records and cannot run until the library is loaded at server start; OSDEV-3236 then flips it to `ddl,role`. To apply that second change without redeploying the application, the **Deploy to AWS** workflow's `deploy-plan-only` boolean was replaced by a `deploy-mode` choice (`full`, `terraform-plan`, `terraform-plan-and-apply`), the last of which stops after `apply`. It pins `image_tag` to the tag the ECS service is already running, because `aws_ecs_service.task_definition` tracks `aws_ecs_task_definition.arn` and the skipped build jobs would otherwise leave the live services pointed at images that were never pushed; normal deploys are unaffected. PostgreSQL logs are still not exported to CloudWatch Logs, so audit records stay in RDS-local files under RDS's default retention. See `doc/ops/database-auditing.md`.
...
* [OSDEV-3225](https://opensupplyhub.atlassian.net/browse/OSDEV-3225) - Floated `js-yaml` (3.15.1 / 4.3.1, GHSA-5p4m-2wfm-xmqj) and `postcss` (8.5.26, CVE-2026-69153) up within their existing ranges via `yarn.lock` only — no new `resolutions` pins, as both consumers already accepted the patched versions.
* [OSDEV-3283](https://opensupplyhub.atlassian.net/browse/OSDEV-3283) - Updated the release protocol so the Preprod anonymized-DB restore (`DB - Apply Anonymized DB`) is monitored with AWS CloudWatch alarms (RDS CPU, connections, free storage, freeable memory, disk queue) as well as BetterStack. `/health-check/` is app liveness only and does not touch Postgres, so BetterStack can stay green while RDS is under restore load. Preprod BetterStack monitors must be **recreated** (not unpaused) after deploy and before the restore: they stay paused while Preprod is down, and BetterStack does not reactivate a paused monitor after a long downtime, so an unpaused leftover would miss restore downtime. Also requires running **Destroy Environment** from the same `releases/v.X.Y` branch that deployed Preprod, not from `main`: Terraform destroy evaluates the selected branch's config (and `destroy.yml`) against Preprod's remote state, and `main` diverges after code freeze, which can fail the destroy or leave orphaned AWS resources.
* [OSDEV-2998](https://opensupplyhub.atlassian.net/browse/OSDEV-2998) - Enabled Route 53 DNS query logging on all four public hosted zones, resolving the Vanta SOC 2 test "Route53 query logging enabled (AWS)". A new `deployment/terraform/route53_query_logging.tf` creates one CloudWatch Logs log group per zone (`/aws/route53/<zone>`, retention from the new `route53_query_log_retention_days` variable, default 365 and validated against the periods CloudWatch Logs accepts) and an `aws_cloudwatch_log_resource_policy` scoped to `arn:aws:logs:us-east-1:<account>:log-group:/aws/route53/*` that lets `route53.amazonaws.com` call `logs:CreateLogStream` and `logs:PutLogEvents`, constrained by `aws:SourceAccount` to block the confused-deputy case. AWS requires both the log group and that resource policy to live in `us-east-1` whatever region the stack runs in, so `config.tf` gains a second us-east-1 provider alias, `aws.us_east_1`, kept separate from the existing ACM-specific `aws.certificates` alias. A public hosted zone can hold only one query logging configuration and several environments point at the same zone inside one AWS account (the constraint the DMARC records hit in OSDEV-2665), so exactly one environment owns each zone. Unlike the DMARC records, ownership is declared in `deployment/environments/terraform-<env>.tfvars` rather than hardcoded in the Terraform: `route53_query_logging_enabled` (default `false`) opts an environment in for the zone named by its `r53_public_hosted_zone`, and `route53_query_log_extra_zones` (default `[]`) covers zones that are not any environment's `r53_public_hosted_zone`. Current owners: `opensupplyhub.org` and `openapparel.org` on `Production`, `staging.opensupplyhub.org` on `Staging`, `os-hub.net` on `Development`; `Test`, `Preprod` and `Rba` share an owned zone and opt out. `os-hub.net` sits with `Development` rather than `Test` because `Development` is deployed automatically on every push to `main` and is not offered by **Destroy Environment**, so its configuration is not torn down and recreated the way `Preprod`'s would be. Moving a zone between environments means flipping the flag in two tfvars files, but the losing environment must be applied before the gaining one: Route 53 rejects a second query logging configuration on a zone, and the log group name would already be taken. Private hosted zones (`osh.internal`) are out of scope - Route 53 query logging supports public zones only. Query logging itself is free, but CloudWatch Logs ingestion and storage for `opensupplyhub.org` is a new recurring cost, so check the first month's bill and lower `route53_query_log_retention_days` if it is material.

### Bugfix
* [OSDEV-3118](https://opensupplyhub.atlassian.net/browse/OSDEV-3118) - Fixed rejected SLC submissions tripping the duplicate-request throttle. `DuplicateThrottle` records a hash of user + body + query params before the view runs and returns `429` ("Duplicate request submitted, please try again later.") for any identical request within 15 minutes. A submission rejected by the duplicate or quality check left that entry behind, so choosing "Go back and edit" and resubmitting the same data was blocked with a raw `429` instead of re-running the checks (the OSDEV-2980 mitigation of hashing query params only covers override resubmits, which hash differently — a plain resubmit is byte-identical). `DuplicateThrottle` gains a `clear()` method, called from `finalize_response` on the `ProductionLocations` viewset for every error response except `429` itself: a rejected request created nothing, so its throttle entry is deleted and an identical retry passes. The entry is still recorded up front, so concurrent identical requests (double-clicks) stay blocked while a request is in flight, and successful submissions keep the full 15-minute protection.
* [OSDEV-3241](https://opensupplyhub.atlassian.net/browse/OSDEV-3241) - Fixed local and CI database image builds against GDAL 3.13. PostGIS 3.4.2 raster compilation referenced GDAL's removed `MIN` macro, so the build now applies the upstream-compatible `Min` replacement before compilation. This does not change the database schema or runtime PostGIS version.
* [OSDEV-2159](https://opensupplyhub.atlassian.net/browse/OSDEV-2159) - Fixed extended fields keeping the original uploader as contributor after a list's `Source.contributor` was reassigned in Django admin. Attribution such as "*<date>* by *<contributor>*" on the production location profile, in facility search responses and in downloads continued to name the previous contributor, because only `Source.contributor` was updated while the `ExtendedField.contributor` rows it produced were left behind. `Source.save()` now detects a contributor change and, once the transaction that changed the source commits (`transaction.on_commit`), re-attributes every `ExtendedField` reached through `FacilityListItem.source` — covering `SINGLE` sources as well as `LIST` sources — via the new `SourceService.reassign_extended_fields`. The re-attribution runs in separately committed chunks of 500 rows, so no single transaction holds locks on `api_extendedfield` or the affected `FacilityIndex` rows for the whole run; each chunk locks the source row and re-reads its current contributor, so callbacks from concurrent reassignments of the same source converge on the latest committed value regardless of execution order; the source row and its extended fields are therefore deliberately not updated atomically, and an interrupted run is finished by re-saving the source or by the backfill command below, since corrected rows stop matching the pending query. The reassignment uses a queryset update on `contributor` only, so `updated_at` is unchanged (the publicly displayed contribution date is preserved) and no new history rows are written, while the existing `api_extendedfield` database trigger refreshes the affected `FacilityIndex.extended_fields` values automatically. Extended fields created from a `FacilityClaim` have no list item and are untouched, and a source whose contributor has been cleared is skipped because `ExtendedField.contributor` is NOT NULL. Saves that do not change the contributor, and saves passing an `update_fields` list without `contributor`, do no extra work.
* [OSDEV-2159](https://opensupplyhub.atlassian.net/browse/OSDEV-2159) - Added the `backfill_extended_field_contributors` management command to repair locations affected before this release. It re-attributes every `ExtendedField` whose contributor no longer matches the contributor of the source it was contributed through, using `bulk_update` on `contributor` only (no `updated_at` change and no new history rows), letting the same database trigger reindex the affected locations. The command commits one batch at a time rather than wrapping the whole run in a single transaction, so no lock is held across a run of this size; it is resumable, idempotent, skips sources with no contributor, and supports `--dry-run`, `--batch-size`, `--limit`, `--sleep` and `--start-after-id`. It is run manually rather than from `post_deployment` — see Release instructions.
* [OSDEV-3219](https://opensupplyhub.atlassian.net/browse/OSDEV-3219) - Fixed the production location detail page (`/production-locations/{os_id}`) showing an infinite loading spinner when the URL used a merged-away OS ID (a facility alias) instead of the surviving target ID. After a merge, the old OS ID remains valid as a redirect target: `GET /api/facilities/{alias}/` returns `302` to the canonical facility and axios loads the target record, but the URL still showed the alias. `ProductionLocationDetailsContainer` treated that ID mismatch as stale in-flight data and never navigated to the canonical URL (a regression from the OSDEV-2373 map work); visiting the target OS ID directly continued to work, which made the bug look limited to specific QA cases. The container now redirects to `/production-locations/{canonical_os_id}` whenever the loaded facility ID differs from the URL, matching the legacy `/facilities/` page behavior. The alias `302` from `GET /api/facilities/{pk}/` now also preserves the trailing slash and original query string (e.g. `created_at_of_data_points`, `pending_claim_info`), avoiding an extra `301` round trip.
...
* [OSDEV-2697](https://opensupplyhub.atlassian.net/browse/OSDEV-2697) - Two SLC (Single Location Contribution) form fixes. The pre-submission validation warning dialog (`ContributionWarningDialog`, which shows the PO Box/short-address and server-returned quality warnings) now renders its warning messages and per-warning titles at 16px instead of the 14px Material-UI default, matching the post-submission "Thanks for adding data" dialog. Each value entered in the Product type(s), Location type(s), and Processing type(s) boxes is now validated against the same 200-character `MAX_STRING_LENGTH` limit already enforced on the Name, Address, and Parent company text fields (a new `max-char-count` test in the SLC array-field validation registry); an oversized value shows a blocking inline error under the field and disables Submit, like the existing Latin-characters and embedded-separator checks.
* [OSDEV-3089](https://opensupplyhub.atlassian.net/browse/OSDEV-3089) - Fixed `clean_match` in the dedupe-hub Gazetteer matcher (`gazetteer_item_match.py`) computing its comparison but never returning it, so the function always evaluated falsy and the exact-string auto-confirm path was dead code: an item with several gazetteer candidates and exactly one exact name/address match was sent to manual review instead of auto-confirming. Also fixed the misspelled `sting_one` parameter. Independent of data centers, but it affects every multi-candidate match.
* [OSDEV-3228](https://opensupplyhub.atlassian.net/browse/OSDEV-3228) - Fixed the sources button being hidden on the production location page for any field with exactly one contribution, which made that contribution's drawer — and therefore its provenance — unreachable. `DataPoint` gated the button on the `contributions` array, which excludes the promoted contribution, so a single-contribution field counted zero. The count now includes the promoted contribution, so such a field reads "1" and opens a drawer showing that contribution with an empty "Other Data Sources" list. Fields with several contributions are unchanged.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Restored the default facility-search sort to `# Contributors` descending after homepage search. The taxonomy release rebuilt the `/facilities` URL from Redux and then re-hydrated filters from that URL; a missing `sort_by` fell back to `name_asc` (`A to Z`) instead of `contributors_desc`. `getAlgorithm` now uses `DEFAULT_SORT_OPTION_INDEX`, matching the Sort By dropdown and the API default when `sort_by` is omitted.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Fixed the Country filter on facility search showing ISO codes (e.g. `US`) instead of display names (e.g. `United States`) after navigating from the homepage or any URL with `countries=` query parameters. `setFiltersFromQueryString` in the React filter actions now restores country labels from loaded country options via `updateListWithLabels`, matching the existing behavior for contributor, parent-company, and list filters.

### What's new
* [OSDEV-3074](https://opensupplyhub.atlassian.net/browse/OSDEV-3074) - Contributors who select **Data Center** as the location type in the single-location contribution (SLC) form now get a data-center-specific layout instead of the production form: the schema's fields are split into collapsible sections (Named Entities, Utility Usage, Operating Information, Building Information, Grouping and Source details), each field carries a short helper description, measures render as a value plus a units input, and yes/no fields such as "Is a group?" render as checkboxes. All fields are optional. Contributions of any other location type see the existing form unchanged.
* [OSDEV-3077](https://opensupplyhub.atlassian.net/browse/OSDEV-3077) - Data-center profiles on the production location page now present their attributes in grouped sections below the shared General Information block, each with an explanatory tooltip, rather than as one flat list. Measures are combined with their units into a single displayed value (for example "20 MW"), empty fields and empty sections are dropped, and `data_center_group_id` renders as a link to the group's profile. Sections appear only when `is_data_center` is true, so production location profiles are unchanged.
* [OSDEV-3227](https://opensupplyhub.atlassian.net/browse/OSDEV-3227) - The same grouped data-center sections now also render on the legacy facility details page (`FacilityDetailsGeneralFields`). Which details page an environment serves is controlled by the `enable_production_location_page` waffle switch, so without this a data center would show none of its data wherever that switch is off. Both pages read the same `DATA_CENTER_FIELD_GROUPS` definition, so the two layouts cannot drift. Gated on `properties.is_data_center`; provenance stays out of scope here, since it is per-row and belongs in the contributions drawer.
* [OSDEV-3073](https://opensupplyhub.atlassian.net/browse/OSDEV-3073) - The contributions drawer on the production location page now shows where a data point came from. Contributions that carry provenance gain a collapsible section listing whichever of the seven fields are present — source name, source link, information source type, date of source, notes, data collection methodology and AI usage notes — with the source link clickable. This applies to the promoted contribution and to the other contributions, including in embed mode. Contributions without provenance render exactly as before, with no extra section.
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - Facility search users can now discover and select Processing Types through autocomplete across OS Hub taxonomy terms and submitted values, see the number of matching locations, and refine suggestions by Facility Type.

### What's new
* [OSDEV-2825](https://opensupplyhub.atlassian.net/browse/OSDEV-2825) - opensupplyhub.org now opens on the new Open Supply Hub homepage instead of going straight into the map. The map has moved to opensupplyhub.org/map, and every link to it inside the app now points there. Existing map links keep working: a request to `/` that carries search parameters redirects to `/map` with the parameters intact, and once inside the app `/` redirects to `/map`.

### Code/API changes
* [OSDEV-1148](https://opensupplyhub.atlassian.net/browse/OSDEV-1148) - The `GET /api/v1/production-locations/` endpoint now returns contribution-related data per the endpoint schema RFC: `contributors` (array of `{id, type}` for publicly-visible contributors), `number_of_contributors` (count of distinct public contributors), and `lists` (array of `{id, contributor_id, name}`). New query capabilities: filter by `contributor_id` and `contributor_type` (both repeatable), and `sort_by=number_of_contributors`. Only contributors from active, public, non-anonymized sources with active matches are exposed — the same visibility rules as the legacy facilities index's `should_display_associations` (anonymized SLC sources are public but identity-masked, so they are excluded here entirely). Existing documents gain the fields only after a full resync of the `production-locations` OpenSearch index (the logstash sync is incremental); until then the new filters return only recently-updated locations. Note the incremental sync keys on `facility.updated_at` only, so contribution changes that do not touch the facility row (a list upload matched to an existing location, or a source later made private/inactive) are reflected in these fields only at the next full resync — a known v1-pipeline limitation tracked in [OSDEV-3131](https://opensupplyhub.atlassian.net/browse/OSDEV-3131).
...
* Ensure that the following commands are included in the `post_deployment` command:
    * `migrate`
    * `backfill_facility_index --fields facility_type,processing_type`
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - The facility and processing type backfill runs after migrations to add historical unmatched contributor-submitted values to `api_facilityindex.facility_type` and `api_facilityindex.processing_type`; it does not rewrite `api_facilityindex.extended_fields`. During the Preproduction release, monitor its runtime and database load before proceeding with Production, and remove it from `post_deployment` after every environment has completed the one-time backfill.
* [OSDEV-2997](https://opensupplyhub.atlassian.net/browse/OSDEV-2997) / [OSDEV-3236](https://opensupplyhub.atlassian.net/browse/OSDEV-3236) - Enable `pgaudit`, per environment. Two Terraform applies with a database reboot after each, and the order is load-bearing: `CREATE EXTENSION pgaudit` must land before `pgaudit.log` is set, or DDL audit records — including this release's own migrations — are written without object type or object name. Every environment runs `rds_multi_az = false`, so each reboot is a hard restart with roughly 30-120 seconds of database downtime rather than a failover; both can run in one maintenance window. Full runbook and verification queries in `doc/ops/database-auditing.md`.
    1. Deploy the release normally (**Deploy to AWS**, `deploy-mode` left at `full`). This applies the OSDEV-2997 commit, where `rds_pgaudit_log` is `none`. `shared_preload_libraries` and `pgaudit.log` land on the parameter group as `pending-reboot`; nothing is audited yet.
    2. Reboot the instance: `aws rds reboot-db-instance --db-instance-identifier opensupplyhub-enc-<env>`. `shared_preload_libraries` is a static parameter and RDS does not apply it on its own, not even during the maintenance window. pgaudit is now loaded with session auditing off.
    3. Run `./deployment/run_cli_task <Environment> "install_db_exts"` to create the extension. The command now exits non-zero if any extension fails, so a green run is the completion signal. Confirm with `SELECT extname FROM pg_extension WHERE extname = 'pgaudit';` and do not continue if it fails.
    4. Run **Deploy to AWS** again with `deploy-mode` set to `terraform-plan-and-apply`, on the OSDEV-3236 commit, which sets `rds_pgaudit_log` to `ddl,role`. This applies the parameter change without rebuilding images or restarting the ECS services. Check the plan in S3 first: it should show the parameter group changing and nothing else — in particular no `aws_ecs_task_definition` or `aws_ecs_service` changes.
    5. Reboot the instance again, then verify: `SHOW shared_preload_libraries;` (expect `rdsutils,pg_stat_statements,pgaudit`), `SHOW pgaudit.log;` (expect `ddl,role`). Run a harmless DDL statement and confirm its audit record carries a populated object type and object name — that is what proves step 3 landed before step 4. Then re-run the Vanta test `aws-rds-pgaudit-enabled` and confirm the environment passes.
* Run the full pgaudit sequence on Development and Staging before Production.
* [OSDEV-1148](https://opensupplyhub.atlassian.net/browse/OSDEV-1148) - The `production-locations` index template gains new mappings (`contributors`, `number_of_contributors`, `lists`), and template changes only apply to a newly created index. When deploying, run the `[Release] Deploy` workflow with `clear-opensearch-target` set to `production-locations` so the index is dropped and rebuilt by the Logstash `sync_production_locations` pipeline, which also backfills the new fields for all existing locations (the incremental sync alone would not). If OSDEV-3115 lands in the same release, one rebuild covers both changes. **The OSDEV-2159 backfill below MUST run before this rebuild**: the incremental sync advances on `facility.updated_at`, which the backfill does not touch, so corrections applied after the rebuild would not reach the index until the next full rebuild. Order: backfill first, then `clear-opensearch-target`.
* `backfill_extended_field_contributors` is a one-time data fix (OSDEV-2159) that is deliberately **not** wired into `post_deployment`. It affects on the order of 400k `ExtendedField` rows, and every updated row fires the existing `api_extendedfield` trigger, which rebuilds the entire `FacilityIndex` summary for that row's location — so a location with 5–6 drifted fields is rebuilt 5–6 times. Run it manually, outside the deployment window, after timing it on Staging:
    * `backfill_extended_field_contributors --dry-run` first, to confirm the affected row and location counts.
    * `backfill_extended_field_contributors --limit 5000` next, to measure throughput on a real sample before committing to the full run.
    * Then the full run, with `--sleep` set to throttle if RDS CPU or connections climb.
* The command commits one batch at a time (`--batch-size`, default 500), so no lock is held across the run and the application stays available throughout. It is resumable: corrected rows stop matching the query, so a plain re-run continues where an interrupted run stopped, and `--start-after-id` skips explicitly past a known id. Because the trigger reindexes as it goes, no `reindex_database` or `backfill_facility_index` call is required afterwards.
* The data-center work does not require a reindex. The new extended-field names are validation metadata only, per-field provenance is resolved at serialization time rather than from the index, and no existing row's indexed values change.
...
* `0223_fix_sector_search_index.py` - Replaces the `index_sector()` SQL function so the searchable `FacilityIndex.sector` column contains only the sectors shown on the location profile: per contributor, the sectors of the single list item the profile displays (items from active sources first, then most recent `updated_at`), plus the latest approved claim's sectors per contributor. Previously the column aggregated the sectors of every list item ever matched to the facility, so superseded values stayed searchable. See OSDEV-992.
...
### Code/API changes
* [OSDEV-2980](https://opensupplyhub.atlassian.net/browse/OSDEV-2980) - Added a duplicate-submission check to the SLC (Single Location Contribution) create flow, to catch a contributor re-submitting the same location and ending up with two production locations. This can happen because the pre-submission "search first" step relies on the OpenSearch `production-locations` index, and a contributor's own just-created submission may not be visible there yet: up to 15 minutes for the (externally hosted) auto-approval automation to approve the pending moderation event, plus up to another 15 minutes for the Logstash pipeline to reindex the approved facility. A new `DuplicateSubmissionProcessor` in the location contribution pipeline queries `ModerationEvent` directly (not OpenSearch) for the same contributor's non-rejected `SLC`/`CREATE` submissions from the last 30 minutes, and flags a match when the country matches exactly, both the name and address clear a 0.9 similarity threshold, and (when present on both sides) every numeric token found anywhere in the name and in the address matches exactly — so a differing street number, suite/unit/building number, or zip code (e.g. the building next door, a different suite in the same building) or a differing unit number in the name (e.g. "Unit 1" vs "Unit 2") isn't masked by an otherwise near-identical string. Rejected submissions are excluded so that resubmitting corrected data after a rejection isn't blocked. On a match, `POST /api/v1/production-locations/` returns `409` with a `duplicate_of` object (`moderation_id`, `created_at`, `name`, `address`, `country`, `duplicate_check_window_minutes`) instead of creating a new moderation event; passing `?duplicate_override=true` as a query parameter (only the literal values `true`/`false` are accepted; anything else returns `400`) bypasses the check. The check itself is also scoped to `SLC`-sourced submissions via `event_dto.source`, so an API `CREATE` is never rejected for matching a recent SLC submission from the same contributor. The SLC form now shows a "Possible Duplicate Submission" confirmation dialog on a `409`, and resubmits with the override flag if the contributor confirms. Scoped to new-location `CREATE` submissions only, not `PATCH` updates to an existing location. Also added a 10-second timeout to `geocode_address` (`api/geocoding.py`), so a hung Google geocoding call can no longer block indefinitely while `DuplicateSubmissionProcessor` holds its per-contributor advisory lock. Since `geocode_address` is shared, this is a deliberate cross-cutting change that also now bounds previously-unbounded geocoding calls made during list upload processing (`geocode_facility_list_item`, run per-row via the `batch_process` management command) and the facility claims geocode-preview endpoint (`GET /api/facility-claims/{id}/geocode/`).
* [OSDEV-2977](https://opensupplyhub.atlassian.net/browse/OSDEV-2977) - Removed the closing date field from the Free GHG Emissions Estimates claim form and from production location profiles, API responses (`claim_info`, v1 `closed_at`), and facility CSV/XLSX downloads (`claim_closing_date`). Opening date and the separate facility closure workflow (`is_closed`) are unchanged. Stored claim closing dates are permanently deleted from the database on deploy. Existing indexed closing dates are cleared via a targeted `claim_info`/`approved_claim` backfill on deploy. Logstash and the production-locations index mapping no longer write `closed_at`; a full OpenSearch rebuild is not required for this release because the v1 API no longer reads or returns the field (any stale values left in the index are unreachable once this code deploys).
...
* [OSDEV-3093](https://opensupplyhub.atlassian.net/browse/OSDEV-3093) - The "Claimed" data attribution (`is_from_claim` on extended fields) now recognizes data the approved claimant contributed through any channel. Previously only fields created directly on the `FacilityClaim` were marked as claim data, so a claimant's SLC or list-upload contributions were labeled "Crowdsourced" on the production location page and in API responses. `FacilityIndexExtendedFieldListSerializer` now also marks a field as claim data when its contributor matches the facility's approved claim contributor (`approved_claim.contributor_id`, passed via serializer context from `get_extended_fields`). Because `is_from_claim` is also a sort key (`_sort_order`), the claimant's contributions are now promoted above other contributors' unverified values, so the top/displayed value of an extended field can change — not just its label. Masked (anonymized) contributions are never attributed to the claim in this release, since the claimant is publicly named; OSDEV-3170 revisits that in 2.29. Partner fields keep the previous behavior (their serializer context does not resolve a claimant). Unclaimed facilities and other contributors' fields are unchanged.
* [OSDEV-3092](https://opensupplyhub.atlassian.net/browse/OSDEV-3092) - Data downloads (`GET /api/facilities-downloads/`) now use the approved claim's address for the `address` column when the claimant has provided one, matching the existing claim-first behavior of the `name` column. Previously the download address ignored claim data entirely, so a claimant's corrected address never appeared in exports. Facilities without an approved claim (or whose claim has no address) are unchanged.
* [OSDEV-2825](https://opensupplyhub.atlassian.net/browse/OSDEV-2825) - Moved the React map application from `/` to `/map`. A `mapRoute` constant was added alongside `mainRoute` in `src/react/src/util/constants.jsx`; `Routes.jsx` now serves the map at `/map` and renders a client-side `<Redirect>` from `/` to `/map` that preserves the incoming location `search` and `state`, so existing map deep links and query-string searches keep working. Every internal link that pointed at `/` now points at `mapRoute`. Where the proxy is enabled, CloudFront serves the Craft homepage at `/`; where it is not (e.g. local development), the React app redirects `/` to `/map`.
* [OSDEV-3094](https://opensupplyhub.atlassian.net/browse/OSDEV-3094) - The claimed-details update endpoint (`PUT /api/facility-claims/{id}/claimed/`) now skips the save when no field value actually changed. Previously every submit re-saved all claim fields unconditionally, bumping `updated_at` (which feeds the claimed section's "last updated" date), firing the claim reindex trigger, rebuilding extended fields, and emailing list contributors — even when the claimant changed nothing. Requests that change at least one value behave exactly as before.

### Architecture/Environment changes
* [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Made `GET /health-check/` app liveness only (HTTP 200, body `ok`) — no django-watchman Postgres/cache checks. Removed `django-watchman`. BetterStack, ALB, and ECS continue to use `/health-check/`; DB degradation is covered by CloudWatch RDS alarms including `DatabaseConnections` via `terraform-aws-postgresql-rds` ≥ 3.3.0. Per-env `rds_database_connections_alarm_threshold` is set to ~80% of each instance’s RDS default `max_connections` (`LEAST(DBInstanceClassMemory/9531392, 5000)`): Dev 90, Staging 720, Test 2880, Preprod/Production/RBA 4000. The 80% mark pages while ~20% headroom remains, so on-call is alerted before new clients fail with “too many connections” under connection pressure. Also retuned absolute FreeableMemory / FreeStorageSpace alarms to ~5% of instance RAM and ~10% of allocated storage per env (Dev 128 MB / 13 GB; Staging 400 MB / 13 GB; Test 1.6 GB / 40 GB; Preprod/Production/RBA 3.2 GB / 25 GB), and raised the shared Memcached FreeableMemory alarm from 10 MB to 500 MB (~16% of `cache.t3.medium`). CPU, DiskQueueDepth, and CPUCreditBalance thresholds stay shared. CloudWatch alarms on `aws_sns_topic.global` (`topic…GlobalNotifications`) go to Slack via AWS Chatbot using `aws_chatbot_slack_team_id` / `aws_chatbot_slack_channel_id` in private `ci-deployment` tfvars; Chatbot uses `ReadOnlyAccess` guardrails and `CloudWatchReadOnlyAccess` on its role. When multiple envs share one AWS account and Slack channel, only one env owns the Chatbot channel config (`aws_chatbot_manage_channel_configuration`): Test owns for Dev/Test/Preprod; Production owns for Production/Staging/RBA. Siblings set it `false`, and the owner attaches their SNS ARNs via `aws_chatbot_additional_sns_topic_arns` in `ci-deployment` (optional, empty-safe for greenfield). `OriginSourceMiddleware` and `DarkVisitorsMiddleware` skip `/health-check/`. See `doc/ops/monitoring.md`.
* [OSDEV-3018](https://opensupplyhub.atlassian.net/browse/OSDEV-3018) - Increased Gunicorn worker count per environment via a new `gunicorn_workers` Terraform variable (replacing the hardcoded `1` in `container_service.tf`). Each environment now uses `(2 × vCPU) + 1` workers — Development 2, Staging/RBA 3, Test/Preprod/Production 5 — raising fleet-wide concurrent request capacity (e.g. Production 5–10 → 25–50 slots) so light requests such as BetterStack's `/health-check/` probe are no longer blocked behind heavy tile/download work. No Fargate task size or count change; workers run within existing task CPU/memory.
...
* [OSDEV-2996](https://opensupplyhub.atlassian.net/browse/OSDEV-2996) - Migrated the React app's build toolchain from `react-scripts` 4 / webpack 4 (Create React App, unmaintained) to **Vite**, keeping Jest as the test runner. This removes the last dependencies locked to the dead CRA 4 tree and clears the associated vulnerability findings that could not otherwise be patched (webpack-dev-server, webpack-dev-middleware, postcss 7.x, transitive `uuid`/`fast-xml-parser`, and the no-patch `ip` and `elliptic` — the latter two leave the dependency tree entirely under Vite rather than being suppressed). The `--openssl-legacy-provider` shim added in OSDEV-2995 is removed (no webpack 4 MD4 hashing on node 22).
  * **Build/dev server:** replaced `@craco/craco` + `react-scripts` with Vite (`vite.config.js`, `@vitejs/plugin-react`, `vite-plugin-svgr`); `yarn start`/`yarn build` now run Vite (`build/` output directory preserved so deploy tooling is unchanged). `index.html` moved to the project root (`%PUBLIC_URL%` → `/`); the dev proxy list moved from `src/setupProxy.js` to Vite `server.proxy`, and `server.allowedHosts` was added because Vite (unlike CRA's dev server) rejects requests whose `Host` header is the `react` compose service name used by the e2e suite. Deleted `craco.config.js`, `setupProxy.js`, `public/index.html`, and `registerServiceWorker.js`.
  * **Tests:** Jest now runs standalone via `jest.config.js` + `babel.config.js` (was `craco test`). Dependencies CRA had provided transitively — `jest`, `eslint-plugin-jest`, `@babel/preset-env`, `@babel/plugin-proposal-class-properties`, `identity-obj-proxy` — are now declared explicitly.
  * **Runtime dependencies replaced** because their published code cannot execute as ES modules under Vite (webpack 4 was silently polyfilling Node built-ins): `object-hash` (abandoned; needs Node `crypto`/`global`) → new dependency-free `src/util/stableHash.js`, and Node's `querystring` module (externalized by Vite) → new `URLSearchParams`-based `src/util/qs.js`. Both preserve prior behavior; the only observable effect is that tile cache keys change format once, so the Django tile cache is cold for a single cycle after deploy (tiles re-render on demand, no user impact).
  * **Build tooling:** bumped `sass` to ^1.101.0 (Vite 8 requires the modern Dart Sass compiler API) and modernized the two SCSS files to `@use 'sass:math'` / `math.div` / `math.percentage`. Aligned `react` and `react-dom` to ^16.14.0 (the release providing `react/jsx-runtime`, required by the automatic JSX transform; `react-dom` was previously pinned to 16.13.1 while `react` already floated to 16.14.0).
  * **Dependency resolutions pruned** from ~55 pins to a minimal set (`braces`, `micromatch`, `@tootallnate/once`, `@babel/runtime`, `node-fetch`, `**/node-notifier/uuid`); the rest existed only for the CRA 4 tree and were removed. The remaining pins were re-derived empirically by removing all overrides, running `yarn audit`, and re-adding only what was still flagged.
...
* [OSDEV-2825](https://opensupplyhub.atlassian.net/browse/OSDEV-2825) - Routed `opensupplyhub.org/` to the new Craft CMS homepage at the CloudFront layer, leaving the platform's own routes untouched. `deployment/terraform/cdn.tf` gains another origin, `originCraft` — a **custom** origin pointing at the Servd-hosted Craft site via the new `craft_cms_origin_domain` variable, plus an `ordered_cache_behavior` with `path_pattern = "/"` targeting it. Both are `dynamic` blocks gated on the new `enable_homepage_proxy` variable (default `false`, with a validation requiring `craft_cms_origin_domain` when enabled). Because `path_pattern = "/"` matches only the exact root path, `/facilities`, `/api/*`, `/contribute`, `/lists`, and every other existing cache behavior are unchanged. Supporting changes:
  * `default_root_object` is now `""` when the proxy is enabled — otherwise CloudFront rewrites `/` to `/index.html` before the ordered cache behavior can run.
  * New CloudFront Function `func{env}HomepageUriRewrite` (`deployment/terraform/cloudfront-functions/homepage_uri_rewrite.js`, viewer-request) rewrites `/` to Craft's `/home-page` entry, and instead 302s to `/map` with the query string re-serialized and preserved when `/` is requested with parameters, so pre-existing map deep links and embedded-map URLs still resolve.
  * The `redirect_to_s3_origin` Lambda@Edge now returns early when `request.uri === '/'` rather than rewriting it to `/index.html`. It is attached to the `default_cache_behavior` only, so it does not run for root requests once the ordered behavior exists; the guard is defense in depth.
  * Root responses use `min_ttl`/`default_ttl`/`max_ttl` of 0 so CMS content edits appear without a CloudFront invalidation.
* [OSDEV-3063](https://opensupplyhub.atlassian.net/browse/OSDEV-3063) - Resolved the AWS MSK advisory reporting topics whose `min.insync.replicas` equals their replication factor. `src/kafka-tools/kafka.sh` created every topic with `--replication-factor 1`, and the MSK configuration in `deployment/terraform/kafka.tf` set no `server_properties` at all, so Kafka's built-in defaults applied (`min.insync.replicas=1`) and RF equalled MinISR — any single broker restart (MSK rolling update, patching, hardware failure) takes those partitions offline and blocks producers using `acks=all`. `server_properties` now sets `default.replication.factor=2` and `min.insync.replicas=1`, and was removed from `lifecycle.ignore_changes`, which had been silently discarding every edit to it. All six environments run 2 brokers across 2 AZs (`aws_availability_zones` is `eu-west-1a`/`eu-west-1b` in every tfvars file), the target is RF=2/MinISR=1, which satisfies MinISR = RF-1 at no additional cost. Supporting changes:
  * `src/kafka-tools/kafka.sh` now creates topics at RF 2 (overridable via `REPLICATION_FACTOR`), no longer re-runs `--create` once per existing topic due to a nested-loop bug, and compares topic names with `grep -qxF` so dotted names are matched literally instead of as regexes.
...
### Bugfix
* [OSDEV-2724](https://opensupplyhub.atlassian.net/browse/OSDEV-2724) - Restored uploaded-file row numbering on facility list pages by fully reverting the earlier OSDEV-2724 change: the header row displays "1" again and `uploadedFileRowIndexOffset` is back to 2, so the page numbers rows exactly as the contributor's file does (header = 1, first data row = 2) and exactly as ContriBot reports and data-team feedback emails cite them. The interim state (offset 1, blank header) made every displayed row one less than the file, so moderators cross-referencing reports/emails against the page were off by one — risking removal of the wrong rows. A proper resolution of the original cosmetic complaint (renumbering the list page and ContriBot together) is deferred to future coordinated work; the constant carries a comment saying the two must change together or not at all.
...
* [OSDEV-3114](https://opensupplyhub.atlassian.net/browse/OSDEV-3114) - Fixed intermittent local stack startup failures on `docker compose up -d`. Kafka in ZooKeeper mode could crash with `NodeExistsException` when the broker container was recreated before ZooKeeper cleared the stale `/brokers/ids/1` znode — migrated local Kafka to single-node KRaft in `docker-compose.yml` and removed the `zookeeper` service. The React development server could also fail when its anonymous `node_modules` volume retained stale dependencies; replaced it with named `react_node_modules` so the local dependency cache has an explicit, inspectable lifecycle and can be reset deliberately. Local dev only; AWS uses MSK and static React builds.
* [OSDEV-1034](https://opensupplyhub.atlassian.net/browse/OSDEV-1034) - Fixed search filtering by Processing Type returning no results for values that are also facility types (e.g. "Final Product Assembly"). Such a value is contributed and displayed under Processing Type on the location profile, but the taxonomy matcher tags its `matched_values` entry `FACILITY_TYPE` (facility type wins), so the old `index_processing_type()` — which kept only entries tagged `PROCESSING_TYPE` — dropped it from the searchable `FacilityIndex.processing_type` column even though filtering by Facility Type worked. The rebuilt function indexes the processing-type value whenever it is present, regardless of the field-type tag, so `GET /api/facilities/?processing_type=...` matches what the profile shows. Also registered a `processing_type` field group for the `backfill_facility_index` command so the column can be refreshed for affected facilities without a full reindex. Raw processing-type text that failed taxonomy matching entirely (no processing-type slot) is intentionally left out of scope — it has never been searchable and cannot be queried from the search page anyway — and is tracked as a follow-up under [OSDEV-3127](https://opensupplyhub.atlassian.net/browse/OSDEV-3127).
* [OSDEV-992](https://opensupplyhub.atlassian.net/browse/OSDEV-992) - Fixed the Sector search filter returning facilities that do not display the searched sector on their profile (e.g. filtering by "Toys" returned facilities whose profiles show no Toys). The profile displays one list item per contributor — the most recent, preferring active sources — plus approved claim sectors, but the old `index_sector()` aggregated the sectors of every list item ever matched to the facility, so a contributor's superseded sectors stayed searchable indefinitely. The rebuilt function mirrors the profile's per-contributor selection, so `GET /api/facilities/?sectors=...` matches what the profile shows. Also registered a `sector` field group for the `backfill_facility_index` command; its filter targets only facilities where a single contributor has more than one matched list item (the only rows whose indexed value can change). The index mirrors the profile's default ordering (`updated_at`); the profile variant requested with `created_at_of_data_points=true` can select a different item per contributor and cannot be represented in the same indexed column.
* [OSDEV-2865](https://opensupplyhub.atlassian.net/browse/OSDEV-2865) - The Delete Facility dashboard page now displays the specific API error message (e.g. "Facilities with approved claims cannot be deleted") instead of the generic "An error prevented deleting that facility".
...
* Ensure that the following commands are included in the `post_deployment` command:
    ...
    * `backfill_facility_index --fields claim_info,approved_claim,processing_type,sector --parallel 10 --batch-size 10000` — one-time targeted backfill that recomputes indexed claim JSON without `closing_date` (OSDEV-2977), refreshes `FacilityIndex.processing_type` after migration `0220` changes `index_processing_type()` (OSDEV-1034), and refreshes `FacilityIndex.sector` after migration `0223` changes `index_sector()` (OSDEV-992). The `processing_type` group only touches facilities that have a `processing_type` `ExtendedField`; the `sector` group only touches facilities where a single contributor has more than one matched list item. The worker count is set per environment via the temporary `BACKFILL_PARALLEL_BY_ENVIRONMENT` map in `post_deployment.py`; remove the call, the map, and the helper after this release has been deployed everywhere.
...
* The combined `backfill_facility_index` operation runs with no application downtime; monitor RDS CPU and connections on the first run after deploy and reduce `--parallel` if needed. Confirm timing on Staging before Production.
...
* After Terraform is applied in each environment, wait for the MSK cluster to report `State: ACTIVE` before running anything against it — the new configuration revision triggers a rolling broker restart, and a partition reassignment started while the cluster is still `UPDATING` will stall because the new replica cannot join the ISR while its broker is cycling. Then, from `deployment/`, run `./run_kafka_cmd <Environment> './fix_replication.sh'` to print the remediation plan, review both sections (partition reassignments and any `min.insync.replicas` repairs — the latter lower durability guarantees on those topics and should not pass unnoticed), and re-run with `'./fix_replication.sh --execute'` to apply. The script verifies its own result and exits non-zero if any topic still violates `min.insync.replicas <= replication factor - 1`, so a clean exit is the completion signal. The reassignment copies partition data between brokers, so run it inside the deployment window and confirm timing on Staging before Production; verification polls for up to 10 minutes (`VERIFY_MAX_ATTEMPTS` in the script). Terraform's new `default.replication.factor` only affects newly created topics, so this remediation is required once per environment.
