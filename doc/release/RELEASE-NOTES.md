...
### What's new
* [OSDEV-3189](https://opensupplyhub.atlassian.net/browse/OSDEV-3189) - On the main facility search page (non-embed), replaced separate Facility Type and Processing Type dropdowns with a combined hierarchical autocomplete and added an ISIC Rev 4 hierarchical search filter. Both fields show facility counts per taxonomy node. A new "Match both facility type and ISIC categories" checkbox lets contributors require both filters to match (AND semantics). Embedded maps keep the existing dropdown filters. Typing text that does not match the taxonomy and clicking Search now commits it as a free-text facility/processing filter chip and searches existing production locations for matching indexed values.
