...
* [OSDEV-2786](https://opensupplyhub.atlassian.net/browse/OSDEV-2786) - Added `anonymise_in_paid_products` boolean column to `api_user` (defaults to `False`). When enabled for a contributor's admin user, that contributor is anonymised in OS Hub's paid products regardless of contributor type. No reindex is required — masking happens at response build time.
