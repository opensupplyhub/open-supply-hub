...
* [Follow-up][OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Ephemeral Preprod no longer lists its SNS ARN in Test's `aws_chatbot_additional_sns_topic_arns` (Chatbot does not backfill subscriptions when the topic is missing at apply time). `deploy_to_aws.yml` runs `deployment/sync_chatbot_sns_topic attach` after Preprod terraform apply; `destroy.yml` runs `detach` before destroy. Companion: ci-deployment removes Preprod from Test tfvars (Dev remains). See `doc/ops/monitoring.md`.
