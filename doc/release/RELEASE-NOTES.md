...
* `0217_add_user_hide_in_paid_products.py` - Adds the `hide_in_paid_products` boolean field (defaulting to `True`) to the `User` model.
...
* [OSDEV-2786](https://opensupplyhub.atlassian.net/browse/OSDEV-2786) - Added `hide_in_paid_products` boolean column to `api_user` (defaults to `True`). When enabled for a trade union (`Union`) contributor's admin user, the contributor is hidden from OS Hub's paid products. No reindex is required — masking happens at response build time.
...
* [OSDEV-2786](https://opensupplyhub.atlassian.net/browse/OSDEV-2786) - Trade union-linked data is now withheld from OS Hub's paid products while staying visible on the public web app. A new `ShouldMaskContribution` service caches the set of `Union` contributors whose admin has `hide_in_paid_products` enabled, matching contributions by contributor `id`, `admin_id`, or name. When a contribution is masked it is relabeled to `Other`, its contributor id is dropped, and its list name/description and other list metadata are stripped. This covers the `contributors` array, extended fields, names, addresses, sectors, claim info, alternate locations, and the `created_from` attribution (matched by name, since that record carries no contributor id). `GET /api/facilities/` (list + detail) masks union contributions for programmatic (token-authenticated) requests only, so the public web app and facility profiles still show union names. The `GET /api/facilities-downloads/` CSV/XLSX export always masks union contributions (web client and API alike). The search-page download button is disabled (with an explanatory tooltip) only when a search targets unions *exclusively* - the **Data Contributor Type** = `Union` filter, a union **Data Contributor**, or that contributor's **Contributor List** sub-filter with no non-union contributor selected. A mixed search - or one using **Show only shared facilities** - stays downloadable with the union rows relabeled to `Other`.
...
* [OSDEV-2786](https://opensupplyhub.atlassian.net/browse/OSDEV-2786) - OS Hub admins can now toggle **Hide contributor name in paid products** per user in Django admin (`/admin/api/user/<id>/change/`), defaulting to on. This keeps trade union contributions visible on facility profiles while hiding the union's identity from the API and data downloads to protect unions and workers.
