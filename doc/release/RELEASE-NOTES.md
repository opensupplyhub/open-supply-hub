...
* [OSDEV-2867](https://opensupplyhub.atlassian.net/browse/OSDEV-2867) - Wired CloudWatch alarms on `aws_sns_topic.global` (`topic…GlobalNotifications`) to Slack via AWS Chatbot. Terraform creates an IAM role plus `aws_chatbot_slack_channel_configuration` when `aws_chatbot_slack_team_id` and `aws_chatbot_slack_channel_id` are set (values live only in private `ci-deployment` tfvars; empty = skip). Guardrail is `ReadOnlyAccess`; the Chatbot role has `CloudWatchReadOnlyAccess` for alarm detail in Slack. See `doc/ops/cloudwatch-alarms-slack.md`. **Post-deploy:** authorize the Slack workspace once per AWS account in the Chatbot console, invite the app to the channel, fill the two IDs in `ci-deployment`, apply, then confirm the SNS topic has a Chatbot subscription and a test ALARM reaches Slack.
