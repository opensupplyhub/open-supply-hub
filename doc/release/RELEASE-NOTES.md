...
### Code/API changes
* Added the India Labour Line helpline as a system partner field (`india_labour_line_helpline`), mirroring the MIT Living Wage pattern but keyed on `Polygon` boundaries instead of US counties: production locations in India whose coordinates fall inside the `india_labour_line_helpline_areas` polygon, and that work in a covered sector (Apparel, Apparel Accessories, Footwear, Textile Mills), get the helpline phone number (read at request time from the partner field's JSON schema `properties.phone_number.default`, so the number is editable in the Django admin without a deploy). The partner's contributor-profile spotlight and the Spotlight Data Partners search filter both resolve via PostGIS polygon containment. Everything is gated behind the `india_labour_line_helpline` waffle switch; missing configuration (polygon or schema) disables the feature with a logged warning rather than erroring or over-matching. Requires no front-end changes.
