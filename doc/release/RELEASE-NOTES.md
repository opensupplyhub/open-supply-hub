...
* [Quickfix][OSDEV-3362](https://opensupplyhub.atlassian.net/browse/OSDEV-3362) - Stopped Dedupe Hub Gazetteer training from crashing on cold start with `Attempting to block with an index predicate without indexing records`. `gazetteer.train()` defaulted to `index_predicates=True`, so Dedupe 1.9.4 applied TF-IDF/Levenshtein blocking rules before `gazetteer.index(canonical)` ran. Training now uses `index_predicates=False` (simple token/n-gram/prefix rules only); canonical facilities are still indexed afterward. A failed initial Gazetteer build now re-raises so the task does not start Kafka and repeat the full-DB dump on every match job.
...
* [Quickfix][OSDEV-3362](https://opensupplyhub.atlassian.net/browse/OSDEV-3362) - After Dedupe Hub starts, confirm CloudWatch shows `Indexing finished` and that match jobs do **not** log `Rebuilding gazetteer` or `Attempting to block with an index predicate without indexing records`. Do not restart the production 2.28 Dedupe Hub task until this quickfix is deployed.
