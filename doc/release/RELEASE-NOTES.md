...
* [OSDEV-2644](https://opensupplyhub.atlassian.net/browse/OSDEV-2644) - **Code Quality** now skips jobs whose trees did not change in the PR, via `dorny/paths-filter` and job-level `if` (not workflow `paths`, so required checks still report). Docs, `deployment/`, and other unmapped paths skip the heavy suites; application changes still run the matching checks (React, Django `api` / `countries` / `contricleaner`, Dedupe Hub, ContriBot, integration). Changing `.github/workflows/code_quality.yml` is the always-run allowlist and forces every job.
