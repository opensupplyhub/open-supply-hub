...
* [OSDEV-1148](https://opensupplyhub.atlassian.net/browse/OSDEV-1148) - Added a `number_of_contributors` field to the OpenSearch-backed `GET /v1/production-locations/` endpoint (list and by-OS-ID responses), as the first slice of the search-bar OpenSearch migration (OSDEV-1575). Added `number_of_contributors` (integer) to the `production-locations` index mapping (`src/logstash/indexes/production_locations.json`) and a correlated-subquery count to the sync query (`src/logstash/sql/sync_production_locations.sql`), plus the field to the `ProductionLocationsResponseMapping` `_source` allowlist. The count is the number of distinct contributors that have an active, non-rejected match (`AUTOMATIC`/`CONFIRMED`/`MERGED`) to the location — this counts all such contributors, not only public/active-source ones, so it will read higher than the legacy `number_of_public_contributors` shown on the current site. Scope note: this ticket's `contributors[]`/`lists[]` arrays and `contributor_id`/`contributor_type` filtering + `number_of_contributors` sorting are deferred to a follow-up. Docs follow-up (in `open-supply-hub-api-docs`): add `number_of_contributors` to the production-location response schema.
...
* [OSDEV-1148] The `number_of_contributors` addition changes the `production-locations` index template. The updated template only applies to a newly created index, and the count is populated by the sync SQL, so recreate/reindex the `production-locations` OpenSearch index (drop + let the Logstash `sync_production_locations` pipeline rebuild it) so existing documents pick up the new mapping and count. Incremental syncs only touch facilities whose `updated_at` has changed, so a full rebuild is required to backfill the field for all locations.
