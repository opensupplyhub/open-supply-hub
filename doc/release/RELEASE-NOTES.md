...
#### Migrations
* `0224_create_named_polygon.py` - Adds the `NamedPolygon` table: a reusable, named geographic boundary (`name`, `description`, and a `geom` `MultiPolygonField` in EPSG:4326) that is not tied to country borders. Polygons may be any shape, including multiple disjoint parts and polygons with holes. `NamedPolygon.facilities()` returns the OS Hub production locations within a given boundary via the existing `location__within` PostGIS lookup. Boundaries are added through the Django admin by pasting or uploading GeoJSON (a bare `Polygon`/`MultiPolygon`, a `Feature`, or a `FeatureCollection`, which are parsed, merged, and validated before saving). This is foundational infrastructure for future boundary-driven features (e.g. per-region partner fields) rather than a user-facing change on its own; related context in [OSDEV-3125](https://opensupplyhub.atlassian.net/browse/OSDEV-3125).
