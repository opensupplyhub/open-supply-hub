...
* [OSDEV-2384](https://opensupplyhub.atlassian.net/browse/OSDEV-2384) - Fixed embedded-map CSV and XLSX downloads from `GET /api/facilities-downloads/` so configured fields use the embed configuration's visibility and order, including visible columns that have no values and excluding hidden columns even when data exists. Mixed-case list-upload headers such as `program_1_Name` now resolve by their original column position, with exact-case matching preferred before a case-insensitive fallback. Standard extended fields remain limited to the embed contributor's data and join all of that contributor's values with `|`, matching existing production download behavior.
