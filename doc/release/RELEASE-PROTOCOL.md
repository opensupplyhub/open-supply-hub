...
4. The responsible person should go to the Databases section of Amazon RDS again and delete the newly created `opensupplyhub-enc-pp` database. After successful deletion, rename `opensupplyhub-enc-pp-current` to `opensupplyhub-enc-pp`.
...
3. On the designated day and time, the responsible person should run the Release [Deploy] workflow. The release branch must be selected, the full release tag version (X.Y.Z) entered, and the environment for deployment chosen. These steps should be replicated for both the production and sandbox environments.
After successfully completing the Release [Deploy] workflow, the 'Deploy to AWS' workflow should automatically initiate for the respective environment.
