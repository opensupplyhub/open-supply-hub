...
3. On the day of the code freeze, the responsible person must run the `Release [Init]` workflow from the `main` branch, specifying the major and minor versions of the release. This action will create the `releases/v.X.Y` branch.
4. On Tuesday, following the code freeze, the responsible person must run the `Release [Deploy]` workflow for the Pre-prod environment from the release branch. 
...
7. In case there is a need to run a command in the terminal of the Django container, follow [this instruction](https://opensupplyhub.atlassian.net/wiki/spaces/SD/pages/140443651/DevOps+Guidelines+for+Migration+Database+Snapshots+and+ECS+Management).
...
In case there is a need to run additional command in the terminal of the Django container, follow [this instruction](https://opensupplyhub.atlassian.net/wiki/spaces/SD/pages/140443651/DevOps+Guidelines+for+Migration+Database+Snapshots+and+ECS+Management).
