...
To choose the right smoke test suite, check the `enable_production_location_page` switch in the Django admin panel: if it is enabled, run the `@smokev2` suite; if it is disabled, run the `@smokev1` suite.
