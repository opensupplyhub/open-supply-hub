...
7. Treat `partner_fields` as a representative set of active partner fields, not an empty object. It is only emitted by `FacilityIndexDetailsSerializer`, so it belongs in the `GET /facilities/{id}/` sample and in matched `POST /facilities/` results, and must NOT appear in default `GET /facilities/` list samples.
   - Keep the full representative set of active partner fields in the `GET /facilities/{id}/` sample and do not trim it back down: `worldly_assessment_data`, `slcp_assessment`, `accord_inspections_and_remediation_program`, `amfori_compliance_status`, `climate_trace_emissions`, `estimated_annual_activity`, `estimated_emissions`, `wage_indicator`, `mit_living_wage`, `rsc_grievance_mechanism`, `labor_solutions_grievance_mechanism`, and `wrap_certification`.
   - Derive the field list and each field's metadata (`label`, `source_by`, `unit`, `base_url`, `display_text`, `json_schema`) from the live `PartnerField` config (`get_cached_all_partner_fields` / the `PartnerField` table), not from memory.
   ...
   - `estimated_annual_activity` may not exist as an active `PartnerField` in every environment. Keep it as an illustrative primitive entry, but note the discrepancy in your report if the local DB does not have it.
