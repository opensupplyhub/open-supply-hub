...
6. Treat `extended_fields` as a full grouped object, not a sparse example:
   ...
   - Omit `duns_id`, `lei_id`, `rba_id`, and `isic_4` from the sample. Even though the serializer emits them as empty arrays in the live response, they are intentionally left out of the documented sample. Include empty arrays for other fields with no values.
...
7. Treat `partner_fields` as a representative set of active partner fields, not an empty object. It is only emitted by `FacilityIndexDetailsSerializer`, so it belongs in the `GET /facilities/{id}/` sample and in matched `POST /facilities/` results, and must NOT appear in default `GET /facilities/` list samples.
   - Keep the full representative set of active partner fields in the `GET /facilities/{id}/` sample and do not trim it back down: `worldly_assessment_data`, `slcp_assessment`, `accord_inspections_and_remediation_program`, `amfori_compliance_status`, `climate_trace_emissions`, `estimated_annual_activity`, `estimated_emissions`, `wage_indicator`, `mit_living_wage`, `rsc_grievance_mechanism`, `labor_solutions_grievance_mechanism`, `ulula_grievance_mechanism`, and `wrap_certification`. Every field in this list is already enabled (active + available in API) on **production**, so keep it in the sample even if it is missing or inactive in your local database.
   - The source of truth for the field list and each field's metadata (`label`, `source_by`, `unit`, `base_url`, `display_text`, `json_schema`) is the **production** `PartnerField` config, not the local environment. The local `PartnerField` table (`get_cached_all_partner_fields` / the `PartnerField` table) is frequently stale or incomplete: it may be missing prod fields or hold outdated metadata. Use local data only as a convenience, and always reconcile against production — if local and production disagree, production wins, and you must call out the discrepancy in your report. Never drop or downgrade a field solely because the local DB lacks it. If you cannot reach production config, treat the list above as the authoritative set and flag that you could not verify against production.
   - Give every partner-field entry the same shape as extended fields, following `FacilityIndexExtendedFieldListSerializer.fields`: `id`, `is_verified`, `value`, `updated_at`, `contributor_name`, `contributor_id`, `value_count`, `is_from_claim`, `field_name`, `verified_count`, `source_by`, `unit`, `label`, `base_url`, `display_text`, and `json_schema`.
   - Match each entry's `value` shape to its `json_schema`: object-typed partner fields use `value.raw_values` (an object); primitive/string fields use `value.raw_value` and set `json_schema` to `null`.
   - Some fields (for example `estimated_annual_activity`) may be absent or inactive in your local database even though they are enabled on production. This is exactly the local-vs-production drift described above: keep the field in the sample as an illustrative entry, trust the production config, and note the discrepancy in your report rather than removing the field.
8. For `POST /facilities/`, preserve the documented outcome headings when present. Make each outcome structurally consistent with the serializer/view code and use a realistic status such as `MATCHED`, `POTENTIAL_MATCH`, or `NEW_FACILITY` only when supported by the code/tests.
