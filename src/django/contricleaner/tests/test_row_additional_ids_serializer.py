...
class RowAdditionalIdsSerializerTest(TestCase):
    ...
    def test_empty_additional_ids_dict(self):
        empty_row = {"additional_identifiers": {}}
        result = self.serializer.validate(empty_row, self.current.copy())
    ...
    def test_additional_ids_not_dict(self):
        invalid_row = {"additional_identifiers": "invalid_value"}
        result = self.serializer.validate(invalid_row, self.current.copy())

        self.assertEqual(len(result["errors"]), 1)
        self.assertEqual(
            result["errors"][0]["message"],
            "Expected value for additional_identifiers to be a dict but "
            "got invalid_value.",
        )
        self.assertEqual(
            result["errors"][0]["field"], "additional_identifiers"
        )
        self.assertEqual(result["errors"][0]["type"], "ValueError")

    def test_additional_ids_key_not_in_allowed_keys(self):
        invalid_row = {"additional_identifiers": {"invalid_key": "value"}}
        result = self.serializer.validate(invalid_row, self.current.copy())

        self.assertEqual(len(result["errors"]), 1)
        self.assertEqual(
            result["errors"][0]["message"],
            "Unexpected key invalid_key in additional_identifiers. The "
            "allowed keys are: duns_id, lei_id, rba_id.",
        )
        self.assertEqual(
            result["errors"][0]["field"], "additional_identifiers"
        )
    ...
    def test_additional_ids_value_not_string(self):
        ...
        self.assertEqual(
            result["errors"][1]["field"], "additional_identifiers"
        )
        ...
        self.assertEqual(
            result["errors"][2]["field"], "additional_identifiers"
        )
        self.assertEqual(result["errors"][2]["type"], "ValueError")

    def test_invalid_duns_id(self):
        invalid_row = {"additional_identifiers": {"duns_id": "12345678"}}
        result = self.serializer.validate(invalid_row, self.current.copy())
    ...
    def test_invalid_lei_id(self):
        invalid_row = {
            "additional_identifiers": {"lei_id": "123456789012345678901234"}
        }
        result = self.serializer.validate(invalid_row, self.current.copy())
