...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        request_types = [ModerationEvent.RequestType.CREATE, 
                         ModerationEvent.RequestType.UPDATE, 
                         ModerationEvent.RequestType.CLAIM]

        statuses = [ModerationEvent.Status.PENDING, 
                    ModerationEvent.Status.RESOLVED]

        sources = [ModerationEvent.Source.API, 
                   ModerationEvent.Source.SLC, 
                   '']

        # Fetch existing contributors and claims
        contributors = list(Contributor.objects.all())
        claims = list(FacilityClaim.objects.all())
        ...
        for _ in range(num_events):
            event = ModerationEvent(
                ...
                status_change_date=datetime.now() if choice([True, False]) else None,
                contributor=choice(contributors) if contributors else None,
                request_type=choice(request_types),
                raw_data={"name": "raw_name", "country_code": "UK"},  # Fill with realistic mock data
                cleaned_data={"name": "cleaned_name",  "country_code": "UK"},
                geocode_result={"latitude": round(randint(-90, 90), 6), 
        ...
        self.stdout.write(self.style.SUCCESS(f"Created {len(events)} mocked moderation events."))
