...
class ApiLimitTest(TestCase):
    def setUp(self):
        ...
        date_limitation_utils = DateLimitationUtils()
        self.now = date_limitation_utils.prepare_start_date(timezone.now())
        ...
        self.notification_time = self.now
    ...
    def test_api_limit_monthly_renewal_period(self):
        ...
        r_log_one.created_at = self.now - relativedelta(months=1)
        r_log_one.save()

        r_log_two.created_at = self.now - relativedelta(months=1)
        r_log_two.save()
    ...
    def test_api_limit_yearly_renewal_period(self):
        ...
        r_log_one.created_at = self.now - relativedelta(years=1)
        r_log_one.save()

        r_log_two.created_at = self.now - relativedelta(years=1)
        r_log_two.save()
