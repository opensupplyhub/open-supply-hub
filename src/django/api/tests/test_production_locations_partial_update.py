...
class TestProductionLocationsPartialUpdate(APITestCase):
    ...
    @patch('api.geocoding.requests.get')
    def test_flagged_user_can_omit_core_fields_on_patch(self, mock_get):
        mock_get.return_value = Mock(ok=True, status_code=200)
        mock_get.return_value.json.return_value = geocoding_data
    ...
    @patch('api.geocoding.requests.get')
    def test_non_flagged_user_omitting_core_fields_gets_422(self, mock_get):
        mock_get.return_value = Mock(ok=True, status_code=200)
        mock_get.return_value.json.return_value = geocoding_data

        self.user.can_partially_update_production_location = False
        self.user.save()
        ...
        minimal_req_body = json.dumps({
            'location_type': 'Coating',
            'product_type': ['nonflagged-unique-1']
        })
