...
class OpenSearchQueryBuilderTest(OpenSearchIntegrationTestCase):
    ...
    def test_query_builder(self):
        ...
        response_match = self.client.search(
            index=self.index_name,
            body=builder_match.get_final_query_body()
        )
        ...
        response_multi_match = self.client.search(
            index=self.index_name,
            body=builder_multi_match.get_final_query_body()
        )
        ...
        response_terms = self.client.search(
            index=self.index_name,
            body=builder_terms.get_final_query_body()
        )
