...
from api.serializers.v1.opensearch_validation_interface import OpenSearchValidationInterface
...
class SearchAfterValidator(OpenSearchValidationInterface):
    def validate_opensearch_params(self, data) -> List[dict]:
        ...
        if len(search_after_values) != 2:
            errors.append({
                "field": "search_after",
                "message": "The search_after parameter must contain exactly two values."
            })
            return errors
