...
VALID_ENVIRONMENTS = (
    "Production", "Staging", "Development", "Test", "Preprod"
)
...
if BATCH_JOB_QUEUE_NAME is None and ENVIRONMENT != "Development":
    raise ImproperlyConfigured("Invalid BATCH_JOB_QUEU_NAME provided, must be set")

BATCH_JOB_DEF_NAME = os.getenv("BATCH_JOB_DEF_NAME")
if BATCH_JOB_DEF_NAME is None and ENVIRONMENT != "Development":
    raise ImproperlyConfigured("Invalid BATCH_JOB_DEF_NAME provided, must be set")

EXTERNAL_DOMAIN = os.getenv("EXTERNAL_DOMAIN")
if EXTERNAL_DOMAIN is None and ENVIRONMENT != "Development":
    raise ImproperlyConfigured("Invalid EXTERNAL_DOMAIN provided, must be set")
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_PERMISSION_CLASSES": ("api.permissions.IsAuthenticatedOrWebClient",),
...
DATA_FROM_EMAIL = os.getenv("DATA_FROM_EMAIL", "data@staging.opensupplyhub.org")

CLAIM_FROM_EMAIL = os.getenv("CLAIM_FROM_EMAIL", "claims@opensupplyhub.org")

NOTIFICATION_EMAIL_TO = os.getenv("NOTIFICATION_EMAIL_TO", "notification@example.com")
...
TILE_CACHE_MAX_AGE_IN_SECONDS = (
    60 * 60 * 24 * 365
)  # 1 year. Also in deployment/terraform/cdn.tf  # NOQA
...
if OAR_CLIENT_KEY is None:
    raise ImproperlyConfigured("Invalid OAR_CLIENT_KEY provided, must be set")
...
ALWAYS_GENERATE_SLOW_REPORT = (
    True  # Generate report only when requested using --slowreport flag
)

KAFKA_BOOTSTRAP_SERVERS = os.getenv(
    "KAFKA_BOOTSTRAP_SERVERS", ""
)  # Kafka servers to connect
KAFKA_TOPIC_DEDUPE_BASIC_NAME = os.getenv(
    "KAFKA_TOPIC_DEDUPE_BASIC_NAME", ""
)  # Kafka Dedupe Hub Topic
