...
SELECT
	...
	(CASE
		...
		SELECT
			1
		FROM
			api_facilitylistitem af6
		LEFT JOIN api_source as5 ON
			af6.source_id = as5.id
		WHERE
			af6.id = base_query.facility_list_item_id
			AND as5.is_anonymized = TRUE
		)
		ELSE FALSE
	END)
