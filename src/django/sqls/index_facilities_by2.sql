...
coalesce((select array_agg(distinct(ac.id)) from api_source as1
...
where fli.facility_id = af.id and ac.id is not null), '{}'), -- contributors_id
...
coalesce((select array_agg(as1.facility_list_id) from api_source as1
	where (as1.id in
		(select afli.source_id from api_facilitylistitem afli where afli.id in
			(select afm.facility_list_item_id from api_facilitymatch afm where afm.is_active) and afli.facility_id = af.id) and as1.is_active and as1.is_public and as1.facility_list_id is not null)),
'{}'), -- lists
'{}', -- custom_text
coalesce((select array_agg(distinct (format_numbers_of_workers((aef.value->'max')::int, (aef.value->'min')::int))) from api_extendedfield aef
where aef.facility_id = af.id and aef.field_name = 'number_of_workers'), '{}'), -- number_of_workers
...
coalesce((select array_agg(distinct (aef.value::TEXT)) from api_extendedfield aef
...
'embed_display_field', (
	select array_agg(aef.display_name) from api_embedfield aef where aef.embed_config_id = ac.embed_config_id and aef.visible
),
'embed_field', (
	select array_agg(aef.column_name) from api_embedfield aef where aef.embed_config_id = ac.embed_config_id and aef.visible
)) as raw from api_facilitylistitem afli
left join api_source as1 on as1.id = afli.source_id
left join api_contributor ac on ac.id = as1.contributor_id
left join api_facilitylist afl on afl.id = as1.facility_list_id
left join api_facilitymatch afm on afm.facility_list_item_id = afli.id
where as1.is_active and afm.is_active and afli.facility_id = af.id
group by ac.id, afli.raw_data, as1.source_type, afli.created_at, afl.header
order by afli.created_at desc) as sqlresult), '{}'), -- custom_field_info
coalesce((select array_agg(json_build_object(
	'id', 	base_query.id,
...
where afli.facility_id = af.id
and (afli.status = 'MATCHED' or afli.status = 'CONFIRMED_MATCH')), '{}'), -- item_sectors
coalesce((select array_agg(json_build_object(
	'sector', afc.sector,
	'created_at', afc.created_at,
	'updated_at', afc.updated_at,
