	...
	where (ac.id in 
		(select as1.contributor_id from api_source as1 where as1.id in 
			(select afli.source_id FROM api_facilitylistitem afli where afli.id in 
				(select afm.facility_list_item_id from api_facilitymatch afm where afm.is_active) and afli.facility_id = af.id) and as1.is_active))),
'{}'), -- contrib_types
...
coalesce(
	...
	'{}'), -- lists
coalesce(
	(select array_agg(distinct(custom_text)) from custom_text(af.id)), 
	'{}'), -- custom_text
...
coalesce((select json_build_object(
'should_display_associations', as1.is_active and as1.is_public,
...
left join api_source as1 on as1.id = afli.source_id
left join api_contributor ac on ac.id = as1.contributor_id
where afli.id = af.created_from_id 
limit 1
...
now(), -- updated_at
coalesce(
	(select string_agg(distinct(custom_text),' ') from custom_text(af.id)), 
	'') -- custom_text_search
FROM api_facility af
