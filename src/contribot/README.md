...
Shared Python dependencies are listed in [`requirements.txt`](requirements.txt) (runtime ranges). CI and local pytest use [`requirements-dev.txt`](requirements-dev.txt), a hashed pin of [`requirements-dev.in`](requirements-dev.in). Refresh with `pip-compile --generate-hashes --output-file src/contribot/requirements-dev.txt src/contribot/requirements-dev.in` (Python 3.13).
