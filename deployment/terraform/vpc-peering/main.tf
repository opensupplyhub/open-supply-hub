...
resource "aws_route" "requester_to_accepter" {
  count = local.is_requester ? length(var.requester_private_route_table_ids) : 0
...
resource "aws_route" "accepter_to_requester" {
  count = local.is_accepter ? length(var.accepter_private_route_table_ids) : 0
...
resource "aws_security_group_rule" "accepter_rds_from_requester" {
  count = local.is_requester && var.create_security_group_rules ? 1 : 0
