...
data "template_file" "export_csv_job_definition" {
  ...
  vars = {
    ...
    google_server_side_api_key_arn          = local.google_server_side_api_key_arn
    oar_client_key_arn                      = local.oar_client_key_arn
    google_service_account_creds_base64_arn = local.google_service_account_creds_base64_arn
  }
}
...
data "template_file" "db_sync_job_definition" {
  ...
  vars = {
    ...
    source_db_port                 = local.source_db_port
    source_db_name                 = local.source_db_name
    source_db_user                 = local.source_db_user
    source_db_password             = local.source_db_password
    ...
    google_server_side_api_key_arn = local.google_server_side_api_key_arn
    oar_client_key_arn             = local.oar_client_key_arn
    email_anonymization_secret_arn = local.email_anonymization_secret_arn
  }
}
