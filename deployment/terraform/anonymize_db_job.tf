module "database_anonymizer" {
  ...
  destination_aws_account       = local.anonymizer_destination_aws_account
  ...
  kms_key_admin_users           = local.anonymizer_kms_key_admin_users
...
module "anonymized_database_dump" {
  ...
  anonymized_database_name                = local.anonymized_database_name
  anonymized_database_username            = local.anonymized_database_username
  anonymized_database_password_secret_arn = local.anonymized_database_password_arn
  ...
  anonymized_database_kms_key_id          = local.anonymized_database_kms_key_id
